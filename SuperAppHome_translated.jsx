...
import {
  ...
  Clock, Users, Percent, Crown, Bell, ArrowRight, Sparkles, Calendar, Car, FileText, Tag
} from 'lucide-react';
...
export default function SuperAppHome() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      axios.get(`${API}/bidblitz-pay/main-balance`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }})
        .then(r => setWalletBalance(Math.round((r.data.bidblitz_balance || 0) * 100))).catch(() => {});
    }
  }, [isAuthenticated]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white pb-20" data-testid="super-app-home">
      ...
      <div className="max-w-lg mx-auto px-4">
        ...
        {isAuthenticated && (
          <Link to="/pay" className="block -mt-3 mb-4 bg-white rounded-2xl shadow-lg p-4 border border-slate-100" data-testid="wallet-card">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-blue-600 font-medium">{user?.bids_balance || 0} {t('bids')}</span>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-bold text-slate-800">{t('exploreDubai')}</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link to="/haendler-finder" className="relative h-32 rounded-2xl overflow-hidden shadow-md group">
              ...
              <div className="absolute bottom-3 left-3">
                <p className="text-white font-bold text-lg leading-tight">{t('dubaiGuide')}</p>
              </div>
            </Link>
            <Link to="/auctions" className="relative h-32 rounded-2xl overflow-hidden shadow-md group">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
              <div className="absolute bottom-3 left-3">
                <p className="text-white font-bold text-lg leading-tight">{t('dubaiAuctions')}</p>
              </div>
            </Link>
          </div>
        </div>
