...
const UserStatsPage = ({ onBack }) => {
  ...
  if (loading) return <div className="min-h-screen bg-[#0A0A0F] flex items-center justify-center" data-testid="user-stats-loading"><Loader2 size={32} className="animate-spin text-[#3B82F6]" /></div>;
  if (!data) return <div className="min-h-screen bg-[#0A0A0F] flex items-center justify-center text-gray-500" data-testid="user-stats-error">{t("stats.error_loading")}</div>;
    ...
    <div className="min-h-screen bg-[#0A0A0F] text-white pb-24" data-testid="user-stats-page">
      <div className="sticky top-0 z-40 bg-[#0A0A0F]/95 backdrop-blur-xl border-b border-white/5 p-4">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-[15px] font-bold">{t("stats.title")}</h1><p className="text-[10px] text-gray-500">{t("stats.subtitle")}</p></div>
        </div>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="p-3 rounded-2xl bg-[#00C2FF]/5 border border-[#00C2FF]/20 text-center">
            ...
            <p className="text-[9px] text-gray-500">{t("stats.balance")}</p>
          </div>
          <div className="p-3 rounded-2xl bg-[#F59E0B]/5 border border-[#F59E0B]/20 text-center">
            ...
            <p className="text-[9px] text-gray-500">{t("stats.coins")}</p>
          </div>
          <div className="p-3 rounded-2xl bg-[#10B981]/5 border border-[#10B981]/20 text-center">
            ...
            <p className="text-[9px] text-gray-500">{t("stats.cashback")}</p>
          </div>
        </div>
        ...
        <div className="bg-[#111118] rounded-2xl p-4 border border-white/5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold">{t("stats.income_vs_expense")}</h3>
          </div>
          ...
          <div className="flex items-center gap-4 mt-3 justify-center">
            <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-[#10B981]" /><span className="text-[9px] text-gray-500">{t("stats.income")}</span></div>
            <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-[#EF4444]" /><span className="text-[9px] text-gray-500">{t("stats.expense")}</span></div>
          </div>
        </div>
        ...
        {data.top_categories?.length > 0 && (
          <div className="bg-[#111118] rounded-2xl p-4 border border-white/5">
            <div className="flex items-center gap-2 mb-3"><PieChart size={16} className="text-[#A855F7]" /><h3 className="text-sm font-bold">{t("stats.top_categories")}</h3></div>
            <div className="space-y-2">
