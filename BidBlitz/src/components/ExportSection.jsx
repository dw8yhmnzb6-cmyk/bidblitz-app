...
const ExportSection = ({ title, exports, t, testIdPrefix = "export" }) => {
  ...
  return (
    <motion.div
      ...
      style={{ background: "rgba(255,255,255,0.96)", border: "1px solid rgba(15,23,42,0.06)" }}
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[13px] font-semibold text-slate-900 font-outfit">{title}</h3>
        </div>
        <motion.button
          ...
          style={{
            ...
            border: `1px solid ${showFilters ? "rgba(0,194,255,0.15)" : "rgba(15,23,42,0.08)"}`,
            color: showFilters ? "#00C2FF" : "#475569",
          }}
          whileTap={{ scale: 0.95 }}
      ...
      {showFilters && (
        ...
        >
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-slate-500 font-medium">{t("export.date_from")}</span>
            <input
              data-testid={`${testIdPrefix}-date-from`}
              type="date"
              value={dateFrom}
              onChange={(e) => setDateFrom(e.target.value)}
              className="bg-white border border-slate-200 rounded-lg px-2 py-1 text-[11px] text-slate-800 outline-none"
            />
          </div>
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-slate-500 font-medium">{t("export.date_to")}</span>
            <input
