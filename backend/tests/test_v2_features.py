...
class TestKidsCheckout:
    ...
    def test_create_kids_checkout_monthly(self):
        ...
        response = self.session.post(
            ...
            json={"plan": "monthly", "origin_url": "https://blitz-driver-taxi.preview.emergentagent.com"}
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
    ...
    def test_create_kids_checkout_yearly(self):
        ...
        response = self.session.post(
            f"{BASE_URL}/api/kids/create-checkout",
            json={"plan": "yearly", "origin_url": "https://blitz-driver-taxi.preview.emergentagent.com"}
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
    ...
    def test_create_kids_checkout_invalid_plan(self):
        ...
        response = self.session.post(
            f"{BASE_URL}/api/kids/create-checkout",
            json={"plan": "invalid_plan", "origin_url": "https://blitz-driver-taxi.preview.emergentagent.com"}
        )
        assert response.status_code == 400, f"Expected 400, got {response.status_code}"
