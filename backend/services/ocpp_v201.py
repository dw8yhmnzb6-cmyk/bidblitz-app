...
async def _remote_stop_at_cap(charge_point_id: str, transaction_id: str, session_id: str) -> None:
    try:
...
async def _update_session_meters(session_id: str, latest_wh: float, latest_w: Optional[float], ts: Optional[str]) -> None:
    ...
    current_cost = _live_session_cost(sess, kwh, ts)
    update = {
    ...
    if cap > 0 and current_cost >= cap and transaction_id:
        ...
        if claimed:
            asyncio.create_task(
                _remote_stop_at_cap(sess["charge_point_id"], str(transaction_id), session_id)
            )
