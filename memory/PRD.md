...
## Current Focus — Controlled Beta Completion Center
- **26.08.2026 Android Release Prep für Google Play gehärtet**: Der Android-Release-Pfad wurde für **Google Play App Signing** vorbereitet. `frontend/android/app/build.gradle` unterstützt jetzt env-gesteuertes **`ANDROID_VERSION_CODE`** / **`ANDROID_VERSION_NAME`**, bricht Release-Builds ohne Signing bewusst früh ab und setzt konsistente Bundle-Einstellungen. Neu ist außerdem `frontend/android/build-release-aab.sh` als **Release Doctor**, der Java 17, Android SDK, Signing, Web-Build, `cap sync android` und `bundleRelease` prüft. Dokumentiert in `frontend/deploy/ANDROID_GOOGLE_PLAY_RELEASE_PREP.md`; `build-mobile-final.sh` und `TESTFLIGHT_PLAY_INTERNAL_TESTING.md` verweisen jetzt ebenfalls auf den Doctor-Skriptpfad.
- **26.08.2026 Android-Blocker sauber verifiziert**: Der AAB-Build ist im aktuellen Preview-Container **weiter nicht direkt erzeugbar**, aber die Ursache ist jetzt erneut sauber belegt: **Linux aarch64/ARM64** trifft auf **x86_64-AAPT2-Binaries** aus AGP 8.7.2 / Android Build Tools. Verifiziert mit `./gradlew bundleRelease` und `./build-release-aab.sh`: Fehler in `:app:mergeReleaseResources` mit **`AAPT2 ... Syntax error: "(" unexpected`**. Ergebnis: Release-Prep ist fertig, das echte uploadfähige `.aab` muss weiterhin auf **x86_64 / Android Studio / CI-Runner** gebaut werden.
- **12.08.2026 Telegram-Monitoring-Backup vorbereitet**: Das Admin-Monitoring unterstützt jetzt zusätzlich ein **Telegram-Backup** für **kritische Alarme** und den **Tagesreport**. Neu sind `backend/services/monitoring_telegram.py` für den Bot-Versand, Telegram-Settings/Test-Endpunkte in `backend/routes/monitoring.py`, zusätzliche Audit-Indizes in `backend/core/database.py` sowie eine neue Telegram-Karte im `frontend/src/pages/MonitoringDashboard.jsx`. Verifiziert mit Backend-Smokes **PASS**, Frontend-Smoke **PASS**, `auto_frontend_testing_agent` **PASS** und `deep_testing_backend_v2` **PASS**. **Wichtig:** Die Zustellung wartet aktuell noch auf echte Backend-Env-Werte für `TELEGRAM_BOT_TOKEN` und `TELEGRAM_CHAT_ID`; bis dahin zeigt die UI korrekt den Wartezustand statt Fehlern.
- **11.08.2026 POS Acceptance Gates final geschlossen**: Die verbleibenden harten POS-Kriterien aus Iteration 314 wurden jetzt technisch und testseitig geschlossen. `frontend/src/pages/MerchantPosSimplePage.jsx` trennt **Failure / Unknown / Review** jetzt explizit, sperrt neue Belastungen bei unklarem Status, nutzt Retry nur über den **zuletzt versuchten Zahlweg**, vergrößert kritische Touch Targets und stabilisiert die Mobile-Sticky-Bar für 320/390/430px. `frontend/src/components/merchant-pos/PosPaymentSheet.jsx`, `PosCartPanel.jsx`, `MerchantPosCustomerDisplayPage.jsx` und `posCopy.js` wurden passend gehärtet. Backend-seitig verhindert `backend/routes/pos_system.py` jetzt auch auf API-Ebene Doppelbelastungen über **`pending_existing`** und **`already_paid`**. Verifiziert via **`/app/test_reports/iteration_316.json` PASS**, zusätzlichem API-Test **`backend/tests/test_pos_acceptance_iter316.py` PASS**, sowie repariertem Legacy-Test **`backend/tests/test_pos_hardening_iter314.py` PASS (16/16)**.
- **10.08.2026 Finance V2 erweitert**: Die Merchant-Finanzstrecke deckt jetzt zusätzlich **Reserve-Regeln**, **Adjustment-Requests mit Freigabe-/Ablehnungslogik**, **Dispute-/Chargeback-Workflows** und **deutschsprachige CSV-Exporte** für Settlements, Payouts, Reserves, Adjustments und Disputes ab. Backend-seitig wurden neue Datenpfade in `backend/services/merchant_settlement.py`, `backend/routes/merchant_settlements.py` und `backend/core/database.py` ergänzt; frontend-seitig erhielten `MerchantCommandCenterPage.jsx`, `AdminMerchantSettlementsPage.jsx` und `frontend/src/services/api.js` die neuen Risiko-/Export-/Admin-Steuerflächen. Verifiziert mit **`pytest backend/tests/test_merchant_finance_v2_iter316.py` PASS (7/7)**, API-Smokes **PASS** und Frontend-Smoke **PASS**.
- **10.08.2026 Kids-GPS-Fehler „Not Found“ behoben**: Die Kids-GPS-API war implementiert, aber nicht mehr im zentralen Router-Register eingetragen. `backend/core/router_registry.py` registriert jetzt wieder **`routes.kids_gps`** und **`routes.kids_gps_websocket`**, wodurch die Endpunkte unter **`/api/kids/gps/*`** wieder live sind. Verifiziert per Live-API-Check auf **`/api/kids/gps/all-locations`** (**200**) und Browser-Smoke ohne sichtbaren **Not Found**-Fehler im Kids-GPS-Flow.
- **02.08.2026 POS Settlement + Daily Closing + Command Center V1 verifiziert**: Die neue Merchant-Finanzstrecke ist jetzt live und testverifiziert. Neu sind **`/api/merchant/balance`** als kanonischer Balance-Endpunkt, **`/api/merchant/command-center`**, Settlement-/Payout-/Daily-Closing-Routen unter `backend/routes/merchant_settlements.py` sowie die Frontend-Routen **`/merchant/dashboard`**, **`/merchant/payouts`**, **`/merchant/pos/daily-closing`**, **`/merchant/settlements/:id`** und **`/admin/merchant-settlements`**. Alle neuen Werte laufen in **Minor Units**; Balance-Ansichten trennen **pending / available / reserved / payout_in_progress / paid_out_total**.
- **02.08.2026 Settlement-/Payout-Backend gehärtet**: `backend/services/merchant_settlement.py` synchronisiert POS-Sales/Refunds in `merchant_balance_entries`, berechnet Fees in Basis Points, unterstützt idempotente Settlement-Berechnung/-Finalisierung, Reserve-Holds/Releases, Daily Closing mit immutable Z-Report-Snapshot und Admin-Audit-Zugriffe auf Settlements/Payouts/Balances. Der anfängliche Syntaxfehler, durch den der Router gar nicht geladen wurde, ist beseitigt; der Router registriert jetzt sauber mit dem restlichen Backend.
- **02.08.2026 Frontend-Routing und Admin-Pfad finalisiert**: Die Merchant-Finanzseiten wurden über `renderSpecialRoutes.jsx` integriert; zusätzlich wurde **`/admin/merchant-settlements`** explizit in `App.js` registriert, nachdem ein Frontend-Testagent diese Route als letzten offenen Punkt gemeldet hatte. Merchant Command Center, Payouts, Daily Closing, Settlement-Detail und Admin Settlement Center laden jetzt ohne Blank-Screen.
- **02.08.2026 Verifikation Merchant Finance V1**: API-Smokes **PASS**, Browser-Smokes **PASS**, **Testing Agent Iteration 315 PASS** (**21/21 Backend-Tests, Frontend 100%**) und zusätzlicher Frontend-/Backend-Zusatzcheck **PASS**. Verifiziert: Merchant-Login, `/merchant/dashboard`, `/merchant/payouts`, `/merchant/pos/daily-closing`, `/merchant/settlements/:id`, `/api/merchant/balance`, `/api/merchant/command-center`, `/api/merchant/payouts`, `/api/merchant/payouts/instant-availability`, `/api/merchant/pos/daily-closing`, `/api/merchant-settlements`, `/api/merchant-settlements/overview` und `/api/admin/merchant-settlements`. **WICHTIG:** KYC bleibt im Preview weiter absichtlich **DISABLED** via `REACT_APP_DISABLE_KYC=true`.
- **02.08.2026 Mehr-Seite gesplittet / Rangliste entfernt**: Die Mehr-Navigation wurde für den Nutzerfluss in **zwei Seiten** aufgeteilt. **Seite 1** zeigt nur die wichtigeren Bereiche, **Seite 2** bündelt ausgelagerte Themen wie **Auktionen, Mining, Rewards, Support und weitere Extras**. Ein sichtbarer **Ranglisten-Eintrag** wurde vollständig aus der Mehr-Seite entfernt.
- **02.08.2026 Route `/leaderboard` absichtlich umgebogen**: Die alte Ranglisten-Ansicht ist nicht mehr Teil der aktiven UX. Wer `/leaderboard` öffnet, landet jetzt in der neuen Mehr-Ansicht direkt auf **Seite 2**, damit kein veralteter Ranking-Flow mehr sichtbar bleibt.
- **02.08.2026 POS-Härtung testverifiziert**: Die neue Merchant-/Customer-POS-Härtung (`/merchant/pos`, Customer Display, Sales, Hardware) wurde durch **Testing Agent Iteration 314** erfolgreich geprüft. Ein kleiner Mobile-Overlay-Punkt an der POS-Sticky-Leiste wurde zusätzlich per Z-Index-Fix entschärft.
- **02.08.2026 Final BidBlitz Completion Phase live abgebildet**: Die neue Admin-Route **`/admin/master-roadmap`** ist jetzt die offizielle Steuerzentrale für die Abschlussphase. Sie enthält **8 Phasen** (P0 Launch Blockers, Core User Flows, Merchant/Admin, Mobile Quality, Translation Audit, Store Safe Release, Release Artifacts, Final Acceptance Report), **Release Gates**, **Feature Registry**, **Launch Readiness**, **CEO View**, **Build-/Environment-/Wallet-Diagnosen** und den **Final Acceptance Report**.
- **02.08.2026 Restriktive Investor-Fortschrittsansicht ergänzt**: Die neue Route **`/investors/progress`** liefert eine bewusst eingeschränkte Fortschrittsansicht ohne Kundendaten, Credentials, Sicherheitsdetails, Quellcode oder offene Schwachstellen. Sie greift auf denselben Master-Roadmap-Datenbestand zu.
- **02.08.2026 Wallet-Konsistenz gehärtet**: `backend/routes/wallet.py` liefert jetzt in **`/api/wallet`** und **`/api/wallet/balance`** explizit **`canonical_source="users.balance"`**; das doppelte `/balance`-Routing wurde bereinigt. Die Master-Roadmap zeigt Wallet-Diagnosen und hält P0 weiter ehrlich offen, bis alle Legacy-Pfade wirklich verschwunden sind.
- **02.08.2026 Visual-QA → Roadmap Synchronisierung**: Neue Visual-QA-Issues werden jetzt zusätzlich in die Master-Roadmap übernommen, inklusive Route, Viewport, Komponente, Screenshot-Referenz, Safe-Autofix-Markierung und Schutzregel für Wallet/Payment/KYC/Auth/Admin-Pfade.
- **02.08.2026 Store-Safe-Release erweitert**: `frontend/src/config/release.js` blockiert im Store-Safe-Modus jetzt zusätzlich sensible Pfade wie **Auktionen, Gaming, Lottery, Mining, Crypto, BNPL und Instant Credit**, damit Mobile-Store-Builds auf den freigegebenen Kern reduziert werden können.
- **02.08.2026 Verifikation Completion Center**: Eigene API-Selbsttests **PASS**, Browser-Smoke **PASS**, Backend-Testdatei **`backend/tests/test_master_roadmap_control_center.py` PASS (2/2)** und **Testing Agent Iteration 313 PASS**. Verifiziert: `/admin/master-roadmap`, `/investors/progress`, Task-/Gate-/Registry-Updates und Wallet-Canonical-Contract. **WICHTIG:** KYC bleibt für Tests global deaktiviert via `REACT_APP_DISABLE_KYC=true`. **MOCKED/ABSICHTLICH DEAKTIVIERT:** KYC ist absichtlich **DISABLED**; einzelne Premium-Upgrades bleiben historisch **MOCKED**.

## Current Focus — Zentrales BidBlitz Design System + Visual QA
- **01.08.2026 Zentrales BidBlitz Design System live**: Unter `frontend/src/design/tokens.js` und `frontend/src/design/tokens.css` existiert jetzt eine gemeinsame Design-Basis für Farben, Typografie, Spacing, Radien, Schatten, Buttonhöhen, Header-/Bottom-Nav-Maße, Safe Areas, Breakpoints und Content-Breiten.
- **01.08.2026 Neue Shared-UI-Bausteine**: `BidBlitzPageShell`, `MoneyAmount`, `CountdownTimer`, `ProductImageGallery`, `BidBlitzButtons` und `BidBlitzCards` wurden als wiederverwendbare Frontend-Bausteine ergänzt. Alle neuen interaktiven und prüfrelevanten Elemente tragen eindeutige `data-testid`-Attribute.
- **01.08.2026 Neue Dev-Dokumentationsroute `/design-system`**: Die neue Seite zeigt Tokens, Typography/Buttons/Cards, Money-/Countdown-Zustände, Gallery-Metadaten und Sticky-CTA-Verhalten. Sie ist bewusst nur als Dev-/Preview-Referenz gedacht und setzt `robots=noindex,nofollow`.
- **01.08.2026 Sichtbare Migration auf Zielseiten**: `/auctions` nutzt jetzt die gemeinsame Page-Shell; `AuctionGridCard.jsx` und `AuctionDetail.jsx` rendern deutsches Währungsformat, neue Gallery-Metadaten, mobile Stack-Regeln und bereinigte deutsche Labels. `/taxi` wurde auf dunkles BidBlitz-Theme, bessere Safe-Area-/Sticky-CTA-Abstände, deutsche Preis-/ETA-Darstellung und übersetzte Regions-Fallbacks gehärtet.
- **01.08.2026 Visual-QA erweitert**: `frontend/tests/visual-qa/visual-routes.spec.cjs`, `frontend/scripts/visual-qa/design-consistency.mjs` und `qa/design-spec.json` prüfen jetzt zusätzlich Design-Token-Snapshots, Button-Mindesthöhen, Gallery-Metadaten und die neue Route `/design-system`.
- **01.08.2026 Verifikation Design System / Auctions / Taxi**: Browser-Smoke **PASS** und **Testing Agent Iteration 312 PASS**. Verifiziert: `/design-system`, `/auctions`, Auktionsdetail und `/taxi` laden ohne Blank-Screen; deutsche Preisformate aktiv; keine verbotenen englischen Labels; **kein Horizontal-Overflow auf 320px / 390px / Desktop**; Bottom-Nav bzw. Taxi-Fullscreen/Safe-Area korrekt. **WICHTIG:** KYC bleibt für Tests global deaktiviert via `REACT_APP_DISABLE_KYC=true`. **MOCKED:** Premium-Upgrades bleiben weiterhin **MOCKED**.
- **01.08.2026 Neue Premium-Homepage-Section „Warum gerade jetzt?“**: Die Guest-Homepage enthält jetzt zusätzlich einen hochwertigen Block mit Headline **„Jetzt beginnt die nächste Entwicklungsphase.“**, Supporting Text, einer **modernen Progress-Timeline** (10 Statuspunkte), **4 Premium-Karten „Warum jetzt?“**, **6 Meilensteinen**, **6 Fokus-Karten** und einer finalen CTA **„Bleibe informiert“**.
- **01.08.2026 CTA-Verhalten „Warum gerade jetzt?“**: Die Buttons sind live und getestet — **„Interesse vormerken“** navigiert nach **`/investieren`**, **„Kontakt aufnehmen“** nach **`/contact`**.
- **01.08.2026 Mehrsprachigkeit „Warum gerade jetzt?“**: `frontend/src/models/homeTranslations.js` wurde erneut erweitert und enthält nun auch die Why-Now-Sektion in allen vorhandenen BidBlitz-Sprachen.
- **01.08.2026 Verifikation Why-Now-Section**: Screenshot-Smoke **PASS** und **Testing Agent Iteration 311 PASS**. Verifiziert: **10 Timeline-Einträge**, **4 Karten**, **6 Meilensteine**, **6 Fokus-Karten**, mobile Responsiveness auf **320px / 390px / 768px / Desktop** ohne Overflow sowie Regression **PASS** für Vision-, Why-BidBlitz- und Investor-Section. Nebenbei wurde ein Frontend-Buildfehler in `AdminPage.jsx` (fehlender `Calendar`-Import) behoben.
- **01.08.2026 Neue Premium-Homepage-Section „Unsere Vision“**: Die Guest-Homepage enthält jetzt zusätzlich eine hochwertige Vision-Sektion mit Headline **„Die Zukunft digitaler Zahlungen beginnt heute.“**, Supporting Text, den Blöcken **„Unsere Mission“**, **„Unsere langfristige Vision“**, **„Unsere Werte“**, **„Warum wir BidBlitz entwickeln“**, **„Unser Anspruch“** und einem finalen CTA **„Gemeinsam die Zukunft gestalten“**. Design: Schwarz/Cyan, große Typografie, Glass-Cards, Motion, mobile-first.
- **01.08.2026 Mehrsprachigkeit „Unsere Vision“**: `frontend/src/models/homeTranslations.js` wurde erneut erweitert und enthält nun auch alle Texte der Vision-Sektion in allen vorhandenen BidBlitz-Sprachen.
- **01.08.2026 Verifikation Vision-Section**: Screenshot-Smoke **PASS**, **Testing Agent Iteration 310 PASS** und zusätzlicher **auto_frontend_testing_agent PASS**. Verifiziert: 4 Missionskarten, 3-Phasen-Roadmap, 6 Werte, 5 Anspruchskarten, funktionierende CTA-Buttons (**Jetzt starten** → Registrierung, **Investor werden** → `/investieren`) sowie **kein Horizontal-Overflow auf 320px / 390px / 768px / Desktop**.
- **01.08.2026 Neue Premium-Homepage-Section „Warum BidBlitz?“**: Die Guest-Homepage erklärt jetzt zusätzlich, warum Kunden und Investoren BidBlitz statt vieler Einzel-Apps nutzen sollten. Neu sind **8 Premium-Feature-Karten**, eine **Vergleichstabelle Andere Apps vs. BidBlitz** und eine **Zielgruppen-Sektion „Für wen ist BidBlitz?“** im bestehenden Schwarz/Cyan-Fintech-Stil mit Glass-Cards, Hover-Effekten und Blue-Glow.
- **01.08.2026 Mehrsprachigkeit „Warum BidBlitz?“**: `frontend/src/models/homeTranslations.js` enthält jetzt auch für diese neue Homepage-Sektion vollständige Übersetzungen in allen vorhandenen BidBlitz-Sprachen. Verifiziert wurde zusätzlich ein englischer Spot-Check.
- **01.08.2026 Verifikation Why-BidBlitz-Section**: Screenshot-Smoke **PASS** und **Testing Agent Iteration 309 PASS**. Verifiziert: korrekte Headline **„Eine App. Viele Möglichkeiten.“**, **8 Karten**, Vergleichsblock, Zielgruppenblock, **keine horizontale Überlaufkante auf 320px/390px** und keine Regression der darunterliegenden Investor-Section.
- **01.08.2026 Premium-Investor-Section auf Homepage ergänzt**: Die Guest-Homepage enthält jetzt zusätzlich eine neue Premium-Sektion **„Warum jetzt in BidBlitz investieren?“** in Schwarz/Cyan mit **6 Informationskarten**, **animierten Kapital-Balken**, **Roadmap 2026/2027/2028+**, finalem CTA und juristisch vorsichtiger Copy **ohne Fake-Zahlen, ohne garantierte Renditen und ohne Wallet-/Karten-Investmentflow**.
- **01.08.2026 Neue öffentliche Route `/investieren` + Interest-Flow**: Neue Seite `frontend/src/pages/InvestierenPage.jsx` rendert denselben Investor-Informationsblock als eigene öffentliche Route und ergänzt einen echten **unverbindlichen Interessens-/Unterlagen-Formularfluss**.
- **01.08.2026 Neues Public-Lead-Backend für Investoreninteresse**: Neue Route `backend/routes/investor_interest.py` mit **`POST /api/investor-interest/lead`** speichert Investor-Interesse getrennt in `investor_interest_leads`, normalisiert E-Mails, schützt gegen Dubletten innerhalb des Fensters und limitiert auf **5 Requests pro IP pro Stunde**. Responses bleiben `_id`-frei.
- **01.08.2026 Mehrsprachigkeit Investor-Sektion**: `frontend/src/models/homeTranslations.js` enthält jetzt vollständige Übersetzungen für die neue Investor-Homepage-/Investieren-Strecke in allen vorhandenen BidBlitz-Sprachen.
- **01.08.2026 Verifikation Investor-Section + Interest-Flow**: Screenshot-Smoke **PASS**, API-Selbsttests **PASS**, Duplicate-Safe-Verhalten **PASS** und **Testing Agent Iteration 308 PASS**. Responsive Verhalten auf **320px / 390px / Desktop** verifiziert. **Nicht gebaut in diesem Schritt:** geschütztes Investor-Login/Register/Portal/Admin-Dokumentenmanagement bleibt der nächste P0-Ausbau.
- **01.08.2026 BidBlitz Pay Sandbox-/Mock-Gateway ergänzt**: Neue Full-Stack-Struktur für **BidBlitz Pay** ist jetzt zusätzlich zum bestehenden Legacy-SDK live, ohne den vorhandenen Checkout zu beschädigen. Implementiert wurden **Zahlung starten**, **Hosted Redirect Checkout**, **BidBlitz-Pay-App-/Wallet-Freigabe**, **Statusprüfung**, **Success-/Cancel-Seiten**, **Webhook-Endpunkt**, **Refund-Struktur**, **Idempotency-Schutz** und **dedizierte Audit-Logs**.
- **01.08.2026 Environment-First für späteren Live-Switch**: Das neue Gateway liest ausschließlich die vorgesehenen Variablen **`BIDBLITZ_PAY_API_URL`**, **`BIDBLITZ_PAY_API_KEY`**, **`BIDBLITZ_PAY_MERCHANT_ID`** und **`BIDBLITZ_PAY_WEBHOOK_SECRET`**. Solange diese nicht vollständig gesetzt sind, läuft das System automatisch im **Sandbox-/Mock-Modus**.
- **01.08.2026 Neue Frontend-Routen für Redirect-Flows**: Unter **`/bidblitz-pay/sandbox`**, **`/bidblitz-pay/checkout/:paymentId`**, **`/bidblitz-pay/success`** und **`/bidblitz-pay/cancel`** existieren jetzt testbare Nutzerflüsse für Redirect, App-/Wallet-Freigabe und Statusanzeige. Die Entwicklerdoku **`/pay/docs`** verweist zusätzlich auf die neue Sandbox.
- **01.08.2026 Backend-Härtung gegen Route-Konflikte**: Der bestehende Pay-SDK-Router wird jetzt vor dem Invoice-Catch-all **`/api/pay/{token}`** registriert, damit **`/api/pay/directory`** und weitere Pay-SDK-Endpunkte nicht mehr versehentlich durch den Invoicing-Pfad abgefangen werden.
- **01.08.2026 Verifikation BidBlitz Pay**: Eigene API-Selbsttests erfolgreich für Create, Idempotency, Status, Mock-Confirm, Cancel, Refund, Webhook und Audit-Logs. Zusätzlich **Testing Agent Iteration 307 PASS**, **auto_frontend_testing_agent PASS** und **deep_testing_backend_v2 PASS**. **MOCKED:** Wallet-/App-Freigabe und Refund-Abschluss im neuen BidBlitz-Pay-Sandbox-Modus sind absichtlich **MOCKED**, bis echte Provider-Zugangsdaten per Environment gesetzt werden.
- **30.07.2026 Mobile Hero-Korrektur nach Video-Feedback**: Die P2P-Homepage-Hero-Demo wurde nach Nutzerfeedback visuell nachgeschärft. Die Phone-Mockups haben jetzt **größere, lesbarere Labels**, eine klarere Hierarchie und einen **mobilen Transfer-Hinweis**, der auf iPhone sauber **zwischen** den beiden Karten sitzt statt überzulagern.
- **30.07.2026 Namenswechsel in der Transfer-Demo**: Die ursprünglichen Demo-Namen **Afrim / Albin** wurden durch **Lena / Jonas** ersetzt, inklusive aller vorhandenen Sprachvarianten in `frontend/src/models/homeTranslations.js`.
- **30.07.2026 Verifikation Mobile Hero-Fix**: iPhone-Smokes erfolgreich, Phone-Mockups nicht abgeschnitten oder überlappend, Transfer-Pill sauber positioniert; finaler **auto_frontend_testing_agent PASS** für die Korrektur.
- **30.07.2026 Homepage Hero auf P2P-Wallet umgebaut**: Die erste sichtbare Section der BidBlitz-Startseite kommuniziert jetzt sofort **Geld senden / Geld empfangen**. Für Gäste ist der bisherige Mining-Fokus durch einen neuen **Wallet-First Hero** ersetzt worden, damit neue Nutzer innerhalb weniger Sekunden verstehen, dass BidBlitz schnelle P2P-Transfers unterstützt.
- **30.07.2026 Neue Wallet-Hero-Inhalte + Transfer-Demo**: Neue Komponente `frontend/src/components/home/P2PHeroSection.jsx` mit Badge **BIDBLITZ WALLET**, Headline **„Geld senden. Geld empfangen. In Sekunden.“**, Support-Text, Trust-Indikatoren und einer animierten Transfer-Demo zwischen **Afrim** und **Albin**. Die Formulierung zur Sofortigkeit wurde korrekt auf **Transfers zwischen BidBlitz Wallet Nutzern** eingegrenzt.
- **30.07.2026 Mining-Proof nach unten verschoben**: Die bisherige Bitcoin-Mining-Proof-Kommunikation wurde in `frontend/src/components/home/HomeMiningTrustPromo.jsx` ausgelagert und deutlich weiter nach unten auf der Homepage platziert. Sie ist dadurch nicht mehr die primäre Hero-Botschaft.
- **30.07.2026 Mehrsprachigkeit + Responsive abgesichert**: `frontend/src/models/homeTranslations.js` enthält jetzt vollständige Übersetzungen für die neue Hero-/How-it-works-Sektion in allen vorhandenen BidBlitz-Sprachen (inkl. Resolve für `en-US`, `sq-XK`, `ar-AE`). Responsive Verhalten wurde gezielt auf **320px**, **390px** und **Desktop** abgesichert.
- **30.07.2026 Verifikation Homepage-Redesign**: Screenshot-Smokes erfolgreich auf 320/390/Desktop, englischer Sprach-Spotcheck erfolgreich, **Testing Agent Iteration 306 PASS**, zusätzlicher finaler **auto_frontend_testing_agent PASS**. Keine Regression bei Login-, Registration-, Wallet- oder Navigationsflows.
- **30.07.2026 In-App Dokumentenvorschau live**: Die Charge-App zeigt hochgeladene **Rechnungen und Garantiebelege jetzt direkt in der App** an. Bilder (JPG/PNG/WebP) öffnen als echte Bildvorschau, PDFs in einer eingebetteten Vorschau mit Download- und „Neu öffnen“-Aktion. Damit entfällt der reine Download-Zwang für Standardfälle.
- **30.07.2026 Vorschau auch im Händlerkontext**: `GET /api/charge-app/merchants/{slug}` liefert jetzt zusätzlich einen **`customer_context`** mit passenden Garantie- und Rechnungsdokumenten des angemeldeten Nutzers für den ausgewählten Händler. Die neue Händlerdetailkarte kann diese Belege direkt mit Vorschau-/Download-Aktionen darstellen.
- **30.07.2026 Frontend-Komponenten für Charge-Dokumente ergänzt**: Neue Komponenten `frontend/src/components/charge/ChargeAttachmentActions.jsx` und `frontend/src/components/charge/ChargeDocumentPreviewModal.jsx` kapseln Preview-/Download-Buttons und die Anzeige für Bild/PDF/Fallback. `ChargeAppPage.jsx` und `ChargeMerchantDetailPage.jsx` nutzen diese Bausteine jetzt konsistent.
- **30.07.2026 Technische Härtung Merchant-Kontext**: `_get_charge_merchants()` in `backend/routes/charge_app.py` merged Profildaten jetzt mit `merchants`, damit **business_name** im Finder mit der Detailseite konsistent bleibt. Dadurch funktionieren Dokumenten-Zuordnung und Händlerkontext zuverlässig.
- **30.07.2026 Verifikation Dokumentenvorschau**: Selbsttests erfolgreich für Blob-Fetch, Invoice-/Warranty-Preview und Merchant-Kontext; Browser-Smoke erfolgreich; **Testing Agent Iteration 305 PASS** mit **22/22 Backend-Tests** und vollständiger Frontend-Verifikation. Keine MOCKED APIs in diesem Block.
- **30.07.2026 Android-AAB-Buildversuch mit neuem Release-Keystore**: Für den vom Nutzer angefragten Play-Store-Build wurde im Workspace ein neuer Release-Keystore unter `frontend/android/bidblitz-upload.jks` erzeugt und `frontend/android/keystore.properties` lokal befüllt. Zusätzlich wurden Java 17, Android commandline tools, `platforms;android-35`, `build-tools;35.0.0` und `platform-tools` im Container nachgezogen.
- **30.07.2026 AAB weiterhin im ARM64-Container blockiert**: Der eigentliche Build (`yarn build`, `npx cap sync android`, `./gradlew bundleRelease`) scheitert weiterhin reproduzierbar an **AAPT2 Exec-Format-/Architektur-Inkompatibilität** im aktuellen **aarch64/ARM64-Container**. Sowohl das Gradle-cached `aapt2-8.7.2-12006047-linux` als auch `build-tools/35.0.0/aapt2` lassen sich hier nicht ausführen. QEMU/Binfmt-Workaround konnte im Container nicht aktiviert werden (`binfmt_misc` mount permission denied). Ergebnis: **kein finales `app-release.aab` in diesem Run**.
- **30.07.2026 Status für Nutzerübergabe**: Signing ist jetzt lokal vorbereitet (Keystore + SHA256 vorhanden), aber der finale Play-Store-AAB muss auf einem **x86_64-Host / Android Studio / lokalem Rechner** gebaut werden. Das bestehende Script `frontend/build-aab-release.sh` bleibt dafür der richtige Einstieg.
- **30.07.2026 Admin-pflegbare Charge-Angebotsregeln live**: Admins können jetzt **Angebotsregeln je Region, Händler und Kategorie** direkt im System pflegen. Diese Regeln steuern die Charge-Personalisierung zusätzlich zur bestehenden Nutzerlogik und erlauben gezielte Priorisierung mit **Score-Boost**, **Priorität**, **Aktiv/Inaktiv-Status** sowie eigener **Angebots-Copy**.
- **30.07.2026 Backend-Regelengine ergänzt**: `backend/routes/charge_app.py` enthält jetzt `ChargeOfferRuleRequest`, geschützte Admin-Endpunkte unter `/api/charge-app/admin/offer-rules*` sowie die Regelanwendung in der Personalisierungsengine. Dashboard-Antworten enthalten jetzt zusätzlich `active_rules_total` und `admin_rules`. Nicht-Admins erhalten auf diese Admin-Endpunkte korrekt **403**.
- **30.07.2026 Frontend-Adminseite ergänzt**: Neue Seite `frontend/src/pages/AdminChargeOfferRulesPage.jsx` für Erstellen, Bearbeiten und Aktivieren/Deaktivieren von Charge-Regeln. Navigation ist sowohl im normalen Admin-Grid (`AdminPage.jsx`) als auch im Full-Admin-Panel (`components/admin/sections.js`) unter **„Charge Angebotsregeln“** eingebunden. `frontend/src/services/api.js` bietet die passenden Admin-Helper.
- **30.07.2026 Verifikation Admin-Regeln**: Selbsttest erfolgreich für Admin-CRUD, Toggle und Dashboard-Auswirkung; Browser-Smoke erfolgreich auf `/admin/charge-offer-rules` und `/charge-app`; **Testing Agent Iteration 304 PASS** mit **22/22 Backend-Tests** und vollständiger Frontend-Verifikation. Keine MOCKED APIs in diesem Block.
- **30.07.2026 Personalisierte Charge-Angebote live**: Die Charge-App spielt jetzt **personalisierte Angebote je Händler/Region** aus. Grundlage sind vorhandene Charge-Daten des Nutzers (Garantien, Rechnungen), Händlerinteraktionen und einfache Produktkategorie-Heuristiken. Die neue Sektion **„Für dich personalisiert“** ist im Dashboard klar von den generischen Angeboten getrennt.
- **30.07.2026 Backend-Personalisierung ergänzt**: `backend/routes/charge_app.py` berechnet jetzt ein **Personalization Profile** mit `region`, `top_merchants`, `merchant_slugs` und `top_categories`, rankt Händler über einen Score und erzeugt `personalized_offers` inklusive deutscher Grundlabels wie **„In deiner Region“**, **„Du hast diesen Händler schon angesehen“** und **„Passend zu deinen Charge-Produkten“**. Zusätzlich existiert `POST /api/charge-app/interactions` zur Erfassung von Merchant-/Offer-Interaktionen.
- **30.07.2026 Frontend-Personalisierung ergänzt**: `frontend/src/pages/ChargeAppPage.jsx` zeigt jetzt Score-Badges, Match-Gründe und CTA-Buttons in der neuen personalisierten Angebotssektion. Der Merchant Finder zeigt zusätzlich `match_reason` pro Händler. Klicks auf Händler und personalisierte Angebots-CTAs werden über `frontend/src/services/api.js` als Charge-Interaktionen getrackt. `ChargeMerchantDetailPage.jsx` protokolliert `merchant_detail_view` beim Laden.
- **30.07.2026 Verifikation Personalisierung**: Backend-Selbsttests erfolgreich für Dashboard, Ranking und Interaction-Tracking; Browser-Smoke erfolgreich für personalisierte Angebotssektion und CTA-Navigation; **Testing Agent Iteration 303 PASS** mit **18/18 Backend-Tests** und vollständiger Frontend-Verifikation. Keine MOCKED APIs in diesem Personalisierungsblock.
- **29.07.2026 BidBlitz Charge V2 geliefert**: Die Charge-Kunden-App wurde direkt um die drei nächsten Nutzerpunkte erweitert: **Datei-Uploads für Rechnungen/Garantiebelege**, **digitaler Garantiepass mit QR + Download** und **Händlerdetailseite direkt aus dem Charge-Finder**.
- **29.07.2026 Sichere Upload-Infrastruktur ergänzt**: Neues Backend-Modul `backend/services/charge_storage.py` integriert Object Storage über den Emergent-Storage-Playbook-Pfad. Uploads laufen jetzt authentifiziert über das Backend, speichern Referenzen in MongoDB und liefern Downloads wieder ausschließlich über Backend-Endpunkte aus. Erlaubte Formate: **PDF, JPG, JPEG, PNG, WebP**.
- **29.07.2026 Charge-V2 Backend-Endpunkte ergänzt**: `backend/routes/charge_app.py` bietet jetzt zusätzlich Upload-/Download-Pfade für Garantie- und Rechnungsbelege, einen **Digitalpass-Endpunkt** unter `GET /api/charge-app/warranty/{registration_id}/pass`, Download unter `.../pass/download` sowie `GET /api/charge-app/merchants/{slug}` für die Charge-Händlerdetailseite. HEAD-Support für Pass-/Attachment-Downloads wurde ergänzt.
- **29.07.2026 Charge-V2 Frontend ausgebaut**: `frontend/src/pages/ChargeAppPage.jsx` zeigt jetzt Dateiauswahl für Garantie/Rechnung, rendert gespeicherte Attachments mit Download-Links, bietet eine sichtbare **Digitalpass-Vorschau mit QR** und öffnet Händler aus dem Finder in eine dedizierte Detailroute. Neue Seite `frontend/src/pages/ChargeMerchantDetailPage.jsx` liefert Kontakt, Highlights, Produkte und Promotions in Charge-Optik.
- **29.07.2026 Verifikation Charge-V2**: Backend-Selbsttests erfolgreich für Upload, Download, Pass und Händlerdetail; Browser-Smoke erfolgreich für Händlerdetail und Charge-Hauptseite; **Testing Agent Iteration 302 PASS** mit **14/14 Backend-Tests** und vollständig verifizierten Frontend-Flows. Keine MOCKED APIs in diesem Charge-V2-Block.
- **29.07.2026 BidBlitz Charge Kunden-App MVP geliefert**: Für Endkunden existiert jetzt ein neues Modul **`/charge-app`** im bestehenden System. Kunden können dort **Garantie registrieren**, **Rechnungen speichern**, **Punkte sehen/sammeln**, **Angebote erhalten** und **Händler finden** – genau entlang des zuletzt bestätigten Nutzerauftrags.
- **29.07.2026 Backend für Charge-Kundenfluss ergänzt**: Neue Route `backend/routes/charge_app.py` mit **`GET /api/charge-app/dashboard`**, **`POST /api/charge-app/warranty/register`** und **`POST /api/charge-app/invoices/save`**. Das Dashboard aggregiert Charge-Garantien, Charge-Rechnungen, loyality-Daten aus dem bestehenden Loyalty-System, Promotions-Angebote sowie Händlerdaten aus `merchant_profiles`.
- **29.07.2026 Frontend für Charge-Kundenfluss ergänzt**: Neue Seite `frontend/src/pages/ChargeAppPage.jsx` mit hochwertiger BidBlitz-Charge-Optik, Hero, Garantieformular, Rechnungsformular, Loyalty-Block, Angebotskarten, Garantie-/Rechnungslisten und Händlerfinder. `frontend/src/App.js` registriert die neue Route **`/charge-app`**; `frontend/src/pages/AllServicesPage.jsx` enthält einen sichtbaren Charge-Eintrag; `frontend/src/services/api.js` bietet die passenden API-Wrapper.
- **29.07.2026 Technische Härtung im Charge-Backend**: Der von der Testing-Iteration gefundene MongoDB-Fehler im Invoice-Save-Pfad wurde direkt behoben (`doc.pop("_id", None)` nach `insert_one`), damit keine ObjectId-Serialisierung in API-Antworten ausbricht.
- **29.07.2026 Verifikation Charge-App MVP**: Browser-Smoke erfolgreich auf `/charge-app`; Garantie-Flow im UI erfolgreich; Backend-Selbsttests für Login, Garantie-Registrierung, Invoice-Save und Dashboard erfolgreich; **Testing Agent Iteration 301 PASS** mit 100% Backend- und Frontend-Erfolgsquote. Keine MOCKED APIs in diesem Charge-MVP.
- **28.07.2026 Händlerportal zweite Ausbaustufe geliefert**: Nach dem V1-Block wurde das Dealer-Portal direkt praxisnäher gemacht. Marketing-Assets besitzen jetzt Download-Links, Händler können Materialanfragen im Portal absenden, und Garantiefälle zeigen nicht nur Listen, sondern auch Detailtext plus direkte Status-Aktionen.
- **28.07.2026 Backend-Erweiterung für operative Händlerarbeit**: `backend/routes/merchant_portal.py` enthält jetzt Download-Endpunkte für BidBlitz-Charge-Branding-/Display-/Creative-Assets, einen Persistenzpfad für `merchant_marketing_requests` und einen Update-Pfad für Warranty-Statuswechsel. Damit entsteht erstmals ein operativ nutzbarer Händlerfluss statt nur statischer Übersichtskarten.
- **28.07.2026 Frontend-Erweiterung Händlerportal V1.5**: `frontend/src/pages/MerchantPortalPage.jsx` bietet im Marketing-Tab Download-Buttons, ein Materialanfrage-Formular und eine Anfragenliste. Der Warranty-Tab zeigt jetzt Fallbeschreibung, Seriennummer, Kundenkontext und Status-Aktionsbuttons (`under_review`, `replacement_sent`, `resolved`).
- **28.07.2026 Verifikation Händlerportal V1.5**: API-Selbsttests erfolgreich für Marketing-Request, Asset-Download und Warranty-Status-Update; Browser-Smoke erfolgreich mit sichtbaren Downloads, Anfrageformular und Garantie-Aktionen; Frontend-Testing-Agent bestätigt die vollständige Funktionsfähigkeit ohne Error Boundary.
- **28.07.2026 Händlerportal V1 für BidBlitz Charge implementiert**: Auf Nutzeranforderung wurde das bestehende Merchant Portal zu einem echten Händlerportal ausgebaut. Händler sehen jetzt im geschützten Bereich die fünf geforderten Arbeitsblöcke **Lagerbestand**, **Nachbestellung**, **Rechnungen**, **Werbematerial** und **Garantieabwicklung**. Das Portal ist bewusst hochwertig ausgerichtet, damit BidBlitz Charge als skalierbare, vertrauenswürdige Hardware-/Retail-Marke wirkt.
- **28.07.2026 Backend-Erweiterung**: `backend/routes/merchant_portal.py` enthält neue Dealer-Endpunkte unter `/api/merchant-portal/dealer/*`. Sie bauen auf vorhandenen POS-, Procurement-, Rechnungs- und Merchant-Daten auf und liefern Inventory-Summary, Purchase-Recommendations, offene Purchase Orders, Invoice-Übersichten, Marketing-Assets und Warranty-Claims. Zusätzlich sind zwei neue Mutationen live: Nachbestell-Entwurf anlegen und Garantiefall erstellen.
- **28.07.2026 Frontend-Erweiterung**: `frontend/src/pages/MerchantPortalPage.jsx` wurde um neue Dealer-Tabs, Premium-Hero, KPI-Metriken, Bestands-/Invoice-/Marketing-/Warranty-Listen sowie Reorder-/Warranty-Formulare erweitert. `frontend/src/services/api.js` bietet die passenden neuen Dealer-Helfer. Alle kritischen Bereiche und Aktionen erhielten eindeutige `data-testid`-Attribute.
- **28.07.2026 Verifikation Händlerportal**: API-Selbsttests erfolgreich auf allen neuen Dealer-Endpunkten; Browser-Smoke erfolgreich mit sichtbaren Tabs für Inventory, Reorders, Invoices, Marketing und Warranty; Frontend-Testing-Agent bestätigt, dass das Händlerportal als nutzbarer erster V1-Block steht und kein Kernproblem offen ist.
- **27.07.2026 Aktueller GitHub-Deploy-Fehler #23 konkret behoben**: Der Nutzer lieferte erstmals den exakten Annotation-Text **`localhost URL found in build`**. Die Ursache war nicht ein echter falscher API-Endpoint im Bundle, sondern ein zu aggressiver Deploy-Guard in `.github/workflows/deploy.yml`, der auf jedes Auftreten des Worts `localhost` in minifizierten JS-Dateien reagierte.
- **27.07.2026 Umsetzung des Build-Guard-Fixes**: Der Workflow blockiert jetzt nur noch bei **echten URL-Leaks** (`http://localhost`, `https://localhost`, `127.0.0.1`, `ws://localhost`, `wss://localhost`) und echten `preview.emergentagent.com`-Origins. Harmlose Textfragmente wie Browser-/Library-Hinweise oder Host-Matcher-Konstanten lösen keinen Fehlalarm mehr aus.
- **27.07.2026 Verifikation dieses Fixes**: Production-Build lokal erfolgreich reproduziert; nach dem Fix melden die Checks **`BAD_LOCALHOST=0`** und **`BAD_PREVIEW=0`**. Damit ist der aktuell gezeigte Fehler im Schritt **Build frontend** gezielt beseitigt. Der nächste GitHub-Run ist jetzt die relevante Bestätigung.
- **27.07.2026 IONOS-Live-Deploy weiter gehärtet**: Nachdem der Nutzer zeigte, dass **CI grün** ist, aber **„Deploy to IONOS Production“** weiter rot bleibt, wurde der Workflow nicht mehr als Build-Thema behandelt, sondern als echtes VPS-Deploy-/Secret-Problem. `.github/workflows/deploy.yml` unterstützt jetzt beide real vorhandenen Varianten der Historie: Passwort-Auth via `VPS_PASSWORD` und Key-Auth via `VPS_SSH_KEY`.
- **27.07.2026 Umsetzung des Auth-Fallbacks im Deploy**: Der Workflow prüft jetzt, ob `VPS_SSH_KEY` gesetzt ist, erzeugt bei Bedarf eine temporäre Key-Datei im GitHub-Runner, schaltet `ssh`/`rsync` dynamisch auf Key- oder Passwort-Modus um und führt MAPBOX-/Restart-Schritte passend mit `appleboy/ssh-action` in getrennten Key-/Password-Varianten aus.
- **27.07.2026 Warum dieser Fix wichtig ist**: In alten Projekt-Dokumenten existierten widersprüchliche Setups (`VPS_SSH_KEY` in älteren Anleitungen, `VPS_PASSWORD` in neuerem Workflow). Dieser Mismatch konnte echte GitHub-Deploy-Runs scheitern lassen, obwohl Build und CI korrekt waren. Der Workflow ist jetzt gegen genau diese Diskrepanz robust.
- **27.07.2026 Startseiten-404s endgültig beseitigt**: Die zuvor bekannten nicht-blockierenden 404s auf **`/api/pro/ads/active`**, **`/api/recommendations/home`** und **`/api/ai/recommendations`** kamen nicht aus defekten Frontend-Komponenten, sondern aus einer unvollständigen Backend-Router-Registrierung. Die Routen waren bereits implementiert in `routes/pro_features.py`, `routes/recommendations.py` und `routes/ai_chat.py`, fehlten aber in `backend/core/router_registry.py`.
- **27.07.2026 Umsetzung des Router-Fixes**: `router_registry.py` registriert jetzt zusätzlich **`routes.recommendations`**, **`routes.pro_features`** und **`routes.ai_chat`**. Dadurch liefern die Home-Module `AdBanner.jsx`, `HomeRecommendations.jsx` und `SmartRecommendations.jsx` wieder normale API-Antworten statt 404.
- **27.07.2026 Verifikation dieses Fixes**: API-Selbsttest erfolgreich mit **200** auf `/api/recommendations/home`, **200** auf `/api/pro/ads/active` und **200** auf `/api/ai/recommendations?limit=4` nach Admin-Login. Zusätzlich **Auto Frontend Testing PASS**: keine 404s mehr, Gast-Homepage lädt normal, `recommendations-section` nach Login sichtbar.
- **24.07.2026 GitHub/IonOS Deploy-Fehler eingegrenzt und Workflow bereinigt**: Auf Basis des Nutzer-Screenshots zum fehlgeschlagenen IONOS-Workflow wurde `.github/workflows/deploy.yml` geprüft. Die reale Schwachstelle war die inkonsistente Frontend-Installations- und Lint-Kette mit Yarn im Deploy-Job, obwohl das Projekt inzwischen stabil über Root-`npm`-Workspaces läuft. Der Workflow nutzt jetzt `setup-node` mit npm-Cache, anschließend `npm ci` im Root, `npm run build` im Frontend sowie `npm --workspace frontend exec -- eslint "src/**/*.{js,jsx}"` für Preflight-Checks.
- **24.07.2026 Verifikation des Deploy-Fixes**: Lokale Nachstellung erfolgreich mit `npm ci`, produktivem Frontend-Build und Workspace-ESLint. Es bestehen weiter viele **Warnings**, aber keine fatalen Fehler in dieser Kette. Damit ist der akute Workflow-Fehlerpfad für den GitHub-Deploy nach IONOS behoben; der nächste echte Beleg ist der erneute GitHub-Run des Nutzers.
- **24.07.2026 Aktueller Fehler „Fehler“ konkret eingegrenzt und behoben**: Die nachgelagerte Nutzer-Meldung wurde nicht pauschal auf iOS/KYC geschoben, sondern auf zwei konkrete Admin-Frontend-Crashes reduziert. `frontend/src/pages/AdminAnalyticsPage.jsx` erwartete ein altes Datenformat und griff auf nicht mehr vorhandene `users.total`-/`revenue`-Strukturen zu. `frontend/src/pages/MerchantAdminPage.jsx` nutzte uneinheitliche Merchant-Felder sowie falsche Endpunkte und konnte bei fehlenden Werten in der Suche auf `undefined.toLowerCase()` laufen.
- **24.07.2026 Umsetzung des Admin-Fixes**: `AdminAnalyticsPage.jsx` kombiniert jetzt die realen Endpunkte **`/api/admin/analytics/overview`** und **`/api/analytics/conversions`**, mappt die Rückgaben auf sichere Karten-/Event-/Feature-Modelle und verwendet konsequente Fallbacks. `MerchantAdminPage.jsx` lädt Händler jetzt über **`/api/admin/merchants/list`**, Detaildaten über **`/api/admin/merchants/{email}/detail`**, normalisiert alle Merchant-Felder defensiv und schützt Aktionen gegen fehlende E-Mails/IDs.
- **24.07.2026 Zweite Ursache im Merchant-Admin beseitigt**: Zusätzlich wurde ein ungewollter Re-Load-Loop entfernt. Der Übersetzungs-Helper `tr` ist jetzt per `useCallback` stabilisiert, sodass `loadMerchants()` nicht dauerhaft neu getriggert wird. Ergebnis: Händlerliste lädt einmal sauber und bleibt sichtbar statt bei „Laden...“ hängen zu bleiben.
- **24.07.2026 Verifikation dieses Fixes**: Browser-Smoke mit Admin-Login erfolgreich; `/admin/analytics` zeigt wieder Statistik-Karten ohne Error Boundary, `/admin/merchants` zeigt wieder mehrere Händlerkarten ohne Crash oder Lade-Loop. Zusätzlich **Auto Frontend Testing PASS** und **Backend-Regression-Check PASS** auf Login, Analytics-Overview, Conversions und Merchant-List. Bekannte **nicht-blockierende** Preview-404s auf **`/api/pro/ads/active`**, **`/api/recommendations/home`** und **`/api/ai/recommendations`** bestehen weiter, sind aber nicht die Ursache des aktuellen Admin-Fehlers.
- **24.07.2026 Testbuild jetzt komplett ohne aktives KYC**: Für die aktuelle Testphase wurde KYC im Frontend nicht nur visuell verborgen, sondern aus allen aktiven Testbuild-Pfaden entfernt bzw. per Feature-Flags neutralisiert. Maßgeblich sind jetzt `KYC_ENABLED=false`, `KYC_REQUIRED=false`, `SHOW_KYC_GATE=false` und `TEST_MODE_FULL_ACCESS=true`. Dadurch rendern weder Home noch Wallet noch More irgendwelche KYC-Sperrkomponenten, und KYC-Routen/Verifikationspfade leiten im Testbuild zurück auf Home statt in KYC-Flows.
- **24.07.2026 Exakte deaktivierte KYC-Pfade**: `HomePage.jsx` rendert die Restriktionskarte nicht mehr, `KYCBanner.jsx` gibt sofort `null` zurück, `AuthPage.jsx` löst keine Pending-KYC-Sonderlogik mehr aus, `useEffectiveKycAccess.js` gibt im Testbuild sofort Freigabe, `App.js` bypassed `/kyc`, `/kyc/status`, `/kyc/start`, `/profile/kyc`, `/verification` und `/test/kyc`, und `MorePage.jsx` verlinkt im Testbuild nicht mehr in einen KYC-Unterpfad.
- **24.07.2026 Verifikation dieses Testbuild-Zustands**: Root-iOS-Bundle neu erzeugt und synchronisiert; **Testing Agent Iteration 298 PASS** bestätigt: keine Texte **„Vor Verifizierung“**, **„Zugriff bewusst reduziert“**, **„KYC Starten“**, **„Identität verifizieren“**, **„Status prüfen“**, **„Erlaubte/Blockierte Bereiche öffnen“** mehr im aktiven Testbuild, Wallet offen, All Services mit 65 Services/8 Kategorien offen, keine `server.url`-Remote-Überschreibung im Capacitor-iOS-Build. **MOCKED:** KYC bleibt im Testbuild absichtlich per Feature-Flags deaktiviert; Premium-Upgrades bleiben **MOCKED**.
- **24.07.2026 RCA für falsche iOS-KYC-Startansicht abgeschlossen**: Die iOS-App lud **lokale** Capacitor-Web-Assets (kein `server.url`-Override in `capacitor.config.ts` bzw. `capacitor.config.json`), aber das Root-Bundle unter **`/app/ios/App/App/public`** war veraltet und hing noch auf **Build `0f408e1-20260719131120`**. Dieses stale Bundle enthielt noch die alte KYC-Sperransicht. Der Fix war daher nicht bloß ein UI-Hide, sondern ein echter Build-/Sync-Fix plus zentralisierte Flag-Logik.
- **24.07.2026 Exakter UI-Auslöser und Guard-Pfad**: Die sichtbare Sperransicht kam aus **`frontend/src/pages/HomePage.jsx`** über `showKycRestrictedExperience`. Zusätzlich blockierte **`frontend/src/App.js`** KYC-restriktierte Pfade via `isKycRestrictedPath(currentPath)` aus **`frontend/src/app/pathUtils.js`**. Beide Pfade hängen jetzt an `SHOW_KYC_GATE` / `KYC_REQUIRED` / `TEST_MODE_FULL_ACCESS`, damit ein Testbuild zuverlässig offen startet, während reales KYC im Code erhalten bleibt.
- **24.07.2026 Build-/Sync-Härtung für iOS**: `frontend/scripts/ios-prepare.js` und `frontend/scripts/ios-sync-web-assets.js` bauen weiterhin das Frontend, synchronisieren aber den nativen Stand jetzt korrekt am Repo-Root per `npx cap sync ios` bzw. `npx cap copy ios`. So wird künftig der tatsächlich verwendete Root-iOS-Ordner aktualisiert statt nur ein Nebenpfad.
- **24.07.2026 Sichtbarer Diagnosepunkt für Admins**: Neue Komponente **`frontend/src/components/TestBuildDebugLine.jsx`** blendet für Admins im Testmodus Build-ID, Commit, Environment, `KYC_REQUIRED`, `SHOW_KYC_GATE`, `TEST_MODE_FULL_ACCESS` und Asset-Quelle ein. Damit lassen sich künftige iOS-Verwechslungen zwischen altem Bundle und aktuellem Build schneller eingrenzen.
- **24.07.2026 Verifikation dieses Fixes**: `npm install` wieder funktionsfähig gemacht, `npm run build` und `npx cap sync ios` erfolgreich ausgeführt, Root-iOS-Version neu auf **`9a49d7e-20260724173048`** synchronisiert. **Testing Agent Iteration 297 PASS** bestätigt: keine Texte **„Vor Verifizierung“**, **„Zugriff bewusst reduziert“**, **„KYC Starten“**, **„Identität verifizieren“**, **„Erlaubte/Blockierte Bereiche öffnen“**, **kein** Live-Check-Banner, Wallet/Home normal offen. **MOCKED:** KYC-Gates sind im Testbuild absichtlich per Feature-Flag deaktiviert; Premium-Upgrades bleiben **MOCKED**.
- **24.07.2026 CI-Workflow auf npm korrigiert**: Der aktuell gemeldete GitHub-/CI-Fehler kam nicht aus dem App-Code selbst, sondern aus einem inkonsistenten Workflow. **`.github/workflows/ci.yml`** nutzt im Frontend-Job jetzt **`npm ci`** statt Yarn und ruft ESLint über den funktionierenden Workspace-Befehl **`npm --workspace frontend exec -- eslint "src/**/*.{js,jsx}"`** auf. Damit endet der Job nicht mehr mit dem vorherigen Musterfehler **„No files matching the pattern '.js,.jsx' were found.“**.
- **24.07.2026 Test-App wieder auf normalen BidBlitz-Start reduziert**: Für den vom Nutzer gewünschten Testzustand wurden die Feature-Flags im Frontend nochmals zentralisiert. `frontend/src/config/testMode.js` exportiert jetzt **`KYC_DISABLED`** und **`SHOW_LIVE_CHECK_BANNER`** zusätzlich zum Testmodus. `HomePage.jsx`, `WalletPage.jsx`, `MorePage.jsx`, `App.js`, `pathUtils.js`, `adminAccess.js` und `KYCBanner.jsx` lesen diesen Zustand einheitlich, sodass Testnutzer ohne sichtbares **Live-Check-Banner**, ohne **Pre-KYC-Gates** und ohne **Wallet-KYC-Banner** starten.
- **24.07.2026 iOS-Build-Skripte auf die Test-Flags erweitert**: `frontend/scripts/ios-prepare.js` und `frontend/scripts/ios-sync-web-assets.js` propagieren jetzt zusätzlich **`REACT_APP_DISABLE_KYC`** und **`REACT_APP_SHOW_LIVE_CHECK_BANNER`** in den Build-Prozess. Dadurch bleibt der gewünschte Testzustand nicht nur im Web-Preview, sondern auch beim nativen iOS-Webbundle konsistent erhalten.
- **24.07.2026 Verifikation dieses Blocks**: Root-`npm ci` erfolgreich, ESLint-Workflow-Command liefert **Exit-Code 0** (weiterhin Alt-Warnungen, aber keine Errors), Browser-Smoke für Reviewer erfolgreich und **Testing Agent Iteration 296 PASS**. Verifiziert wurden explizit: `live_banner=0`, `pre_kyc_home=0`, `pre_kyc_more=0`, `wallet_kyc_banner=0`. **MOCKED:** KYC-Gates bleiben im Testbetrieb absichtlich über Feature-Flags verborgen; Premium-Upgrades bleiben weiterhin **MOCKED**.
- **24.07.2026 Vollständiges Root-Exportprojekt `BidBlitz/` erstellt**: Für den Download wurde ein vollständiges Root-Projekt unter `/app/BidBlitz` aufgebaut und als ZIP unter `/app/BidBlitz.zip` verpackt. Enthalten sind **`package.json`**, **`package-lock.json`**, **`capacitor.config.ts`**, **`src/`**, **`public/`**, **`ios/`**, **`backend/`**, **`README.md`**, **`.env.example`** sowie die nötigen Build-/Capacitor-Dateien. Es ist **kein leerer SwiftUI-Stub** und **nicht nur der ios-Ordner**.
- **24.07.2026 Export-Projekt für Production bereinigt**: Produktionswerte im Export zeigen jetzt auf **`https://bidblitz.ae`**, Bundle Identifier bleibt **`com.bidblitz.app`**, und Export-Konfigurationsdateien enthalten **keine echten Secrets** mehr. Dafür wurden u. a. `.env.production`, `backend/.env.production`, `capacitor.config.live.ts`, `public/version.json` und `backend/build_info.json` bereinigt.
- **24.07.2026 Export-Verifikation**: Im Root-Export wurden erfolgreich **`npm install`**, **`npm run build`**, **`npx cap sync ios`** und **`npx cap open ios`** ausgeführt. Zusätzlich verifiziert: **`ios/App/App.xcworkspace`** vorhanden, **`import Capacitor`** in `AppDelegate.swift`, Bundle Identifier **`com.bidblitz.app`** im Xcode-Projekt. **Testing Agent Iteration 294 PASS**. Ein echter **Xcode-/iOS-Build** bleibt **nicht verifizierbar in Linux**, da dafür **macOS + Xcode + CocoaPods** nötig sind.
- **23.07.2026 KYC-Feedback und manuelle Prüfung ausgebaut**: Der Kunde sieht bei KYC-Ablehnung jetzt **konkrete deutsche Fehlergründe** statt nur generischem Fehlschlag, z. B. **Vorderseite zu hoch/zu niedrig**, **abgeschnitten**, **unscharf**, **zu dunkel**, **Spiegelung**, **Selfie nicht klar**, **Gesicht passt nicht**. Dafür wurde die Gemini-Vision-Ausgabe in `backend/services/kyc_ai_verifier.py` um strukturierte Felder wie `front_issues`, `back_issues`, `selfie_issues` und `user_feedback` erweitert und in `backend/routes/kyc.py` auf über 30 konkrete Nutzertexte gemappt. Zusätzlich zeigt jetzt auch die **KYC-Review-/Absenden-Seite** (`frontend/src/pages/KYCFlow.jsx`) diese Gründe **direkt vor dem erneuten Absenden** an, statt nur den generischen roten Balken **„Übermittlung fehlgeschlagen“**.
- **23.07.2026 Farbige Fehlerklassifizierung pro Bild**: Für **Vorderseite**, **Rückseite** und **Selfie** gibt es jetzt eine eigene visuelle KYC-Klassifizierung mit `KYCImageIssueGrid.jsx`: **Rot = Fehler**, **Gelb = Warnung/noch keine Bewertung**, **Grün = aktuell kein Problem erkannt**. Die Klassifizierung ist in **KYC Review**, **KYC Status** und **VerificationPage** eingebunden und zeigt dem Kunden sofort, welches Bild konkret betroffen ist.
- **23.07.2026 Admin-Notiz, Nachupload und Live-Warnung ergänzt**: Admins können in der KYC-Liste jetzt pro Fall eine **freie Notiz** schreiben und mit **`decision = reupload`** gezielt einen **Nachupload** anfordern. Der Kunde sieht diese Hinweise anschließend in **KYCFlow** und **VerificationPage** als klare Reupload-Karte. Zusätzlich prüft `frontend/src/utils/kycImageInspector.js` Bilder bereits **vor dem Absenden** auf einfache Qualitätsprobleme wie **zu dunkel** oder **zu niedrige Auflösung** und zeigt sofortige Warnhinweise pro Upload-Feld an.
- **23.07.2026 Live-Overlay direkt auf Vorschaubildern**: Auf den KYC-Bildvorschauen erscheinen jetzt direkte Overlay-Badges **am Bild selbst** — sowohl im **Upload-Schritt** als auch im **Review-Schritt**. `KYCFlow.jsx` nutzt dafür `PREVIEW_OVERLAY_STYLES` + `getPreviewOverlayMeta()`. Ergebnis: **Rot = Fehler**, **Gelb = Hinweis**, **Grün = OK** ohne Umweg über separate Textblöcke.
- **23.07.2026 Manuelle KYC-Prüfung nach 2 Fehlversuchen**: Schwelle auf **2 Fehlversuche** gesetzt (`MANUAL_REVIEW_THRESHOLD = 2`). Danach erscheint im Nutzerflow ein Button **„Manuelle Prüfung anfordern“**; `POST /api/kyc/manual-review/request` legt den Fall in die Admin-Prüfung. Die Admin-KYC-Ansicht zeigt jetzt **Fehlversuche**, **konkret erkannte Probleme** und Buttons zum **Freischalten / Ablehnen**. Wichtig: **TEST_MODE** bleibt aktiv für Feature-Zugänge, aber die KYC-Statusseite zeigt intern trotzdem den **echten KYC-Status** und nicht pauschal `approved`.
- **23.07.2026 Verifikation KYC-Ausbau**: Selbsttest für kompletten Nutzer- und Admin-Ablauf erfolgreich (Registrierung → rejected mit 2 Fehlversuchen → manuelle Prüfung anfordern → Admin approve). Zusätzlich **Testing Agent Iteration 289 PASS**: Backend 12/12 Tests erfolgreich, Frontend 100%, Manual-Review-Button, detaillierte Feedback-Liste und Admin-Freigabe verifiziert. Danach noch **Iteration 290 PASS** speziell für den Nutzer-Screenshot-Bug: Die Review-/Absenden-Seite rendert jetzt `kyc-review-feedback-card`, `kyc-review-feedback-item-*` und bei Bedarf `kyc-review-manual-review-btn` mit den konkreten Fehlerursachen.
- **23.07.2026 Verifikation Bild-Klassifizierung**: **Auto Frontend Smoke PASS** + **Testing Agent Iteration 291 PASS**. Verifiziert wurden `verification-image-issue-grid`, `kyc-status-image-issue-grid` und die state-dependent `kyc-review-image-issue-*` Test-IDs samt korrekter Farblogik pro Bild.
- **23.07.2026 Verifikation Nachupload & Live-Warnung**: **Backend-Selbsttest PASS** für `decision = reupload` (Admin setzt Nachupload, Kunde sieht `reupload_requested = true` und `admin_note`). Zusätzlich **Testing Agent Iteration 292 PASS**: Admin-Notizfeld, Nachupload-Button, Kundenkarten (`kyc-admin-reupload-card`, `verification-admin-reupload-card`) und state-dependent Live-Warnungen `kyc-live-warning-*` vollständig geprüft.
- **23.07.2026 Verifikation Live-Overlay-Markierung**: **Auto Frontend Smoke PASS** + **Testing Agent Iteration 293 PASS**. Verifiziert wurden `kyc-preview-overlay-front/back/selfie` im Upload und `kyc-review-preview-overlay-front/back/selfie` im Review. Overlays sind korrekt versteckt ohne Vorschau und erscheinen sichtbar mit **„Hinweis“** bei problematischen Bildern.
- **22.07.2026 GitHub-Deploy-Workflow-Fehler behoben**: Der aus dem Nutzer-Screenshot gemeldete Fehler **`.github/workflows/deploy.yml: No jobs were run`** wurde auf die bisherige Einschränkung `on.push.branches: [main]` zurückgeführt. `deploy.yml` triggert jetzt auf **jeden Push** und entscheidet in einem neuen Schritt **`run_mode`** sauber zwischen **Branch-Preflight** (Build + ESLint + Backend-Smoke, aber kein Live-Deploy) und **echtem Deploy** (nur bei `workflow_dispatch` oder `refs/heads/main`). Alle VPS-/SSH-Schritte sind jetzt explizit per `if: steps.run_mode.outputs.should_deploy == 'true'` geschützt.
- **22.07.2026 Verifikation des Workflow-Fixes**: YAML-Syntax lokal geprüft, Preview-Smoke erfolgreich geladen und **Testing Agent Iteration 288 PASS** bestätigt: kein `main`-only Push-Filter mehr, Branch-Runs enden nicht mehr leer, `workflow_dispatch` und `main` behalten den echten Deploy-Pfad, Heredoc-/Einrückung im Deploy-Step ist gültig.
- **19.07.2026 Finaler Produktions-Readiness-Timing-Fix**: Die neuen Live-Logs zeigten, dass zwar der Hintergrund-Bootstrap lief, aber **`Application startup complete`** noch nicht früh genug kam. Der letzte Blocking-Punkt war der sofort startende Router-Bootstrap direkt im Startup-Fenster. In `backend/server.py` wurde deshalb am Start von `_bootstrap_worker_routes_and_tasks()` ein kleines **`await asyncio.sleep(0.1)`** ergänzt, damit Uvicorn den Startup-Zyklus und Port-Bind zuerst abschließen kann, bevor das schwere Hintergrundladen beginnt.
- **19.07.2026 Verifikation des finalen Live-Fixes**: **Testing Agent Iteration 281 PASS** bestätigt: 12/12 Backend-Checks grün, `/health` antwortet sofort, 5 aufeinanderfolgende Health-Checks ohne `connection refused`, Mehrworker-Start mit `/health` innerhalb ~2s, und das Produktionssymptom wird direkt adressiert.
- **18.07.2026 Live-Deploy-Readiness verifiziert**: Nach dem Produktionsproblem („Preview gut, Live bleibt alt“) wurde der Backend-Startpfad nicht nur angepasst, sondern auch getestet. **Testing Agent Iteration 280 PASS** bestätigt: `register_all_routers(app)` blockiert den Import nicht mehr, `/health` ist direkt nach Import vorhanden, Importzeit liegt bei ~**0.395s**, und Nicht-Health-Routen werden während `booting` korrekt mit `503` geschützt, bis die App vollständig bereit ist.
- **Wichtiger Live-Hinweis**: Die Produktionsseite bleibt bis zum **nächsten Redeploy** weiterhin auf dem alten Stand. Der Fix ist jetzt im Code verifiziert, aber die alte Live-Auslieferung ersetzt sich erst nach erneutem Deploy durch den neuen Build.
- **18.07.2026 Guest-/Landing-Mobile-Polish abgeschlossen**: `HomePage.jsx`, `LandingPage.jsx` und `CookieBanner.jsx` wurden für iPhone-Viewports weiter verdichtet. Guest-Header nutzt jetzt `flex-wrap` und kleinere Guest-Buttons; der Cookie-Banner ist auf Mobilgeräten kompakter und klarer über der Bottom-Nav positioniert; `/landing` wurde im Hero mit kleineren Logo-/Titel-/CTA-/Badge-Abständen mobil sauberer gemacht.
- **18.07.2026 Verifikation des Guest-/Landing-Polish**: **Testing Agent Iteration 279 PASS** bestätigt auf **390x844** und **320x568**: kein horizontaler Overflow, Login/Register sichtbar, Cookie-Banner kompakt und getrennt von der Bottom-Nav, `/landing`-Hero komplett innerhalb des Viewports.
- **18.07.2026 Chatbot-Dateien physisch gelöscht**: Nach der bereits erfolgten Render-Entfernung wurden die unbenutzten Dateien **`AIChatWidget.jsx`**, **`FloatingChatbot.jsx`**, **`LandingChatbot.jsx`** und **`LandingChatbot.css`** vollständig aus `frontend/src/components/` gelöscht. Übrig bleibt nur noch die Legacy-Storage-Bereinigung in `index.js`, damit alte Browser-Einträge nicht nachwirken.
- **18.07.2026 Verifikation der Dateilöschung**: **Testing Agent Iteration 278 PASS** bestätigt: alle 4 Dateien fehlen tatsächlich, es gibt keine Referenzen mehr in `frontend/src`, und die Landingpage lädt ohne Chatbot-Elemente.
- **18.07.2026 Alte Widget-Speicher aktiv bereinigt**: Für den Verdacht auf „alte Speicher“ wurden die beiden relevanten Legacy-Keys **`bb_ai_chat_session`** und **`bidblitz-chatbot-hidden`** identifiziert und in `frontend/src/index.js` per `purgeLegacyWidgetStorage()` beim App-Start vor React aktiv entfernt.
- **18.07.2026 Verifikation der Widget-Storage-Bereinigung**: **Testing Agent Iteration 277 PASS** bestätigt: beide Keys werden nach Reload und frischer Navigation zuverlässig gelöscht; zusätzlich sind keine Chatbot-FAB-/Sheet-Elemente mehr sichtbar.
- **18.07.2026 Chatbot-Komponenten komplett aus dem Rendering entfernt**: Auf ausdrücklichen Nutzerwunsch werden **`AIChatWidget`**, **`LandingChatbot`** und **`FloatingChatbot`** nicht mehr gerendert. In `App.js` wurden die Render-Stellen für `AIChatWidget` und `LandingChatbot` vollständig entfernt; in `LandingPage.jsx` wurde das direkte `LandingChatbot`-Rendering ebenfalls gelöscht. `FloatingChatbot` ist aktuell nirgendwo mehr eingebunden.
- **18.07.2026 Verifikation der vollständigen Entfernung**: **Testing Agent Iteration 276 PASS** bestätigt: keine Imports/Renders dieser drei Komponenten mehr in `frontend/src`; auf Guest-Landing, `/landing` und eingeloggtem Home erscheinen keine Floating-Chatbot-/AI-Buttons mehr.
- **18.07.2026 Produktions-Start für Backend entschärft**: Da die Live-Seite (`bidblitz.ae`) noch einen älteren Stand zeigte, wurde der Backend-Startpfad auf Deployment/Readiness geprüft. Ergebnis: der riskante Teil lag nicht im Frontend-Button selbst, sondern in der Live-Auslieferung. `backend/server.py` beantwortet `/health` jetzt frühzeitig und verschiebt schwere Initialisierung (Indexes, Seeds, Legacy-Cleanup, Scheduler/Loops) in einen **Hintergrund-Post-Startup-Task** mit **Cross-Worker-Lock**. Dadurch soll Kubernetes die App schneller als „ready“ sehen, statt beim Booten auf schwere Startarbeiten zu warten.
- **18.07.2026 Health-/Startup-Status erweitert**: `/health` liefert jetzt zusätzlich `startup_status` (`booting`/`ready`/`error`), damit klarer ist, ob der Server schon antwortet, während Hintergrund-Initialisierung noch läuft. Das hilft besonders bei Live-/Produktions-Probes.
- **18.07.2026 iPhone-Header- und Bottom-Nav-Abstände weiter nachgeschärft**: `HomePage.jsx` nutzt im mobilen Header jetzt robustere Flex-Abstände mit `min-w-0`, `truncate`, `shrink-0` und klaren Gaps zwischen Avatar/Name und den rechten Controls. In `App.css` wurde die Bottom-Nav zusätzlich an iPhone-Safe-Areas angepasst: mehr linkes/rechtes Padding, mehr Bottom-Padding via `env(safe-area-inset-*)` und größere Tap-Zonen für `nav-item` und `nav-center`.
- **18.07.2026 Verifikation des iPhone-Spacing-Fixes**: **Testing Agent Iteration 275 PASS** bestätigt: kein Header-Overlap, 12px Abstand zwischen linken/rechten Header-Blöcken, 15.2px Seitenpadding und 16px Bottom-Padding in der Bottom-Nav sowie saubere Sichtbarkeit von Sprache/Benachrichtigung auf 390x844.
- **18.07.2026 Echte Ursache des lila Floating-Buttons geklärt**: Der Button war **nicht primär ein Speicherproblem**, sondern kam aus **mehreren Render-Pfaden gleichzeitig**: `App.js` (`AIChatWidget`, `LandingChatbot`), `LandingPage.jsx` (direktes `<LandingChatbot />`) und als weitere Legacy-Option `FloatingChatbot.jsx`. Zusätzlich existierten nur Begleit-Keys wie `bb_ai_chat_session` (Chat-Session) und `bidblitz-chatbot-hidden` (Hide-State), aber **nicht** die Sichtbarkeit selbst als alleinige Quelle. Jetzt greifen doppelte Schutzschichten: **Komponenten geben bei `TEST_MODE` direkt `null` zurück** und CSS blockiert zusätzlich alle bekannten Floating-Selektoren auf `html.test-mode-active` und `body.test-mode-active`.
- **18.07.2026 Verifikation des Floating-Button-Fixes**: **Testing Agent Iteration 274 PASS** bestätigt: auf `/` und `/landing`, mobil und desktop, sind `ai-chat-fab`, `floating-chatbot-bubble`, `.chatbot-toggle-btn`, `.chatbot-window` sowie die AI-/KI-Aria-Buttons vollständig verschwunden. Testbericht meldet außerdem: **„0 fixed purple elements found“**.
- **18.07.2026 Alte Frontend-Token-Logik vollständig bereinigt**: Die verbleibenden `localStorage.getItem('token')`-Aufrufe in `AdminLandingLeadsPage.jsx`, `LiveKitStreamPage.jsx`, `POSHardwareModal.jsx`, `AgeVerificationModal.jsx` und `POSRetailEnterpriseComponents.jsx` wurden entfernt. Diese Requests laufen jetzt einheitlich über **Cookie-Auth mit `credentials: 'include'`**. Ergebnis: keine alte Bearer-Token-Logik mehr im regulären Frontend-Auth-Pfad.
- **18.07.2026 Verifikation der Token-Bereinigung**: **Testing Agent Iteration 273 PASS** bestätigt: **kein** `localStorage.getItem('token')` mehr in `frontend/src`, keine `Authorization`-/`Bearer`-Patterns mehr in den betroffenen Dateien, App-Smoke-Test grün. Einzig `child_token` in `ChildModePage.jsx` bleibt als **separater Child-Feature-Token** bestehen und gehört nicht zur normalen Auth-Session.
- **18.07.2026 Legacy-Auth-Speicher aktiv bereinigt**: Der alte lokale Mock-Key **`bidblitz_auth`** war zwar laut Code-Suche nicht mehr zentrale Auth-Quelle, konnte aber Altzustände verfälschen. `frontend/src/services/authService.js` nutzt jetzt echte Backend-Auth via `api`, und eine neue Cleanup-Funktion **`purgeLegacyAuthStorage()`** wird beim App-Start (`frontend/src/index.js`), im `UserContext.jsx` sowie in allen Auth-Methoden aufgerufen. Ergebnis: alte lokale Mock-Sessions werden aktiv entfernt, bevor React rendert oder Auth-Flows laufen.
- **18.07.2026 Verifikation des Speicher-Fixes**: **Testing Agent Iteration 272 PASS** bestätigt, dass `localStorage['bidblitz_auth']` nach frischer Navigation und nach Reload zuverlässig entfernt wird. Die Landingpage lädt normal weiter, und `authService.js` enthält keine Mock-Delay-/Local-User-Patterns mehr.
- **18.07.2026 Lokale Speicherorte zentral dokumentiert**: Zur Ursachenanalyse wurde eine neue Übersicht **`/app/memory/STORAGE_MAP.md`** angelegt. Darin sind alle aktuell relevanten `localStorage`-/`sessionStorage`-Keys, ihre Quelldateien und ihre Zwecke dokumentiert. Wichtiges Ergebnis: **Auth, aktueller User und KYC werden nicht lokal gespeichert**, sondern kommen aus **Cookie-Session + `api.getMe()` / `api.refresh()` + `/api/kyc/status`**. Als auffälliger Legacy-Rest bleibt nur `frontend/src/services/authService.js` mit `bidblitz_auth`, der laut aktueller Code-Suche nicht aktiv genutzt wird.
- **18.07.2026 Floating AI-/Chat-Button im Testmodus entfernt**: Auf direkten Nutzerhinweis wurde der lila schwebende AI-/Chat-Button aus dem Testbetrieb entfernt. `App.js` rendert jetzt sowohl **`AIChatWidget`** (eingeloggt) als auch **`LandingChatbot`** (Gastmodus) nur noch bei **`!TEST_MODE`**. **Testing Agent Iteration 270 PASS** bestätigt: `ai-chat-fab` = 0, `.chatbot-toggle-btn` = 0 und keine Layout-Brüche auf mobilen Routen.
- **18.07.2026 Finaler KYC-Gate-Root-Cause behoben**: Die letzten iPhone-Screenshots zeigten, dass Home/More trotz aktivem Testmodus vereinzelt noch die Pre-KYC-Karten rendern konnten. Die echte Ursache war eine **veraltete/stale lokale User-State-Ableitung** im Frontend, obwohl der Server bereits `approved` zurückgab. Dafür wurde `frontend/src/hooks/useEffectiveKycAccess.js` eingeführt: Der Hook fragt live `/api/kyc/status` ab und gibt das Ergebnis als **`forceKycUnlocked`** an `HomePage.jsx`, `MorePage.jsx` und `WalletPage.jsx` weiter. Dadurch verschwinden die Karten **„Zugriff bewusst reduziert“** und **„Vor KYC nur Basisbereiche sichtbar“** selbst dann zuverlässig, wenn lokale Statusreste noch alt sind.
- **18.07.2026 Verifikation des finalen KYC-Gate-Fixes**: **Testing Agent Iteration 269 PASS** bestätigt: `pre-kyc-home-gate` = 0, `pre-kyc-more-gate` = 0, die Literaltexte sind nicht mehr im DOM vorhanden, und Wallet bleibt für den Reviewer vollständig zugänglich. **Keine MOCKED APIs** — die Freischaltung basiert auf der echten `/api/kyc/status`-Antwort.
- **18.07.2026 TEST_MODE zentral eingeführt und verifiziert**: Die App besitzt jetzt einen **einzigen globalen Schalter `TEST_MODE=true`** in `frontend/.env` und `backend/.env`. Im Frontend läuft der Bypass zentral über `frontend/src/config/testMode.js`; im Backend/Auth über `backend/core/config.py` + `backend/core/security.py`. Ergebnis: Testnutzer werden bei aktivem Testmodus automatisch wie **voll KYC-verifiziert** behandelt, KYC-Banner/-Dialoge/-Redirects bleiben verborgen, und Finanz-/Commerce-/Merchant-Flows bleiben freigeschaltet, **ohne** das KYC-System aus dem Code zu entfernen. Beim späteren Umschalten auf `TEST_MODE=false` greift der normale KYC-Flow wieder.
- **18.07.2026 Dashboard-Karte „Zugriff bewusst reduziert“ effektiv deaktiviert**: Da die Home-/More-Gates jetzt nur noch außerhalb von `TEST_MODE` rendern, ist die komplette Pre-KYC-Card im Dashboard/Testbetrieb verschwunden. Gleichzeitig bleiben die ursprünglichen Komponenten (`pre-kyc-home-gate`, `pre-kyc-more-gate`, `KYCBanner`) unverändert im Code vorhanden und werden nur bedingt unterdrückt.
- **18.07.2026 Build-/QA-Status für TEST_MODE**: `yarn build` PASS (nur Alt-Warnungen), `yarn ios:diagnose` PASS, `npx cap sync android` PASS. **Testing Agent Iteration 264 PASS** bestätigt: `/api/auth/me` liefert `kyc_status=approved`, `/api/kyc/status` ist freigeschaltet, Home-/More-KYC-Gates sind weg, Wallet ist ohne Redirect erreichbar und die mobilen Wallet-Karten bleiben klickbar.
- **18.07.2026 Wallet-Mobile-Tap-Bug final behoben**: Nach neuem Nutzerhinweis („hier kann man nicht drauf drücken“) wurde `frontend/src/pages/WalletPage.jsx` für iPhone-/Mobile-Taps nachgeschärft. Die unteren Wallet-Karten **„Kasse / Händler“**, **„Privat empfangen“** und **„Privat senden“** nutzen jetzt native `<button>`-Elemente statt `motion.button`, eine gemeinsame `handleWalletRouteNavigate(...)`-Weiterleitung, `touchAction: "manipulation"`, `WebkitTapHighlightColor: "transparent"` sowie `relative z-10`, damit keine Overlay-/Pointer-Interception den Tap schluckt. Parallel wurden die oberen Quick-Buttons ebenfalls auf denselben sicheren Navigationspfad gezogen. **Testing Agent Iteration 263 PASS** bestätigt **6/6 Wallet-Taps** auf iPhone-Viewport (390x844) erfolgreich.
- **18.07.2026 iOS-CoreData-Launch-Härtung erweitert**: Zusätzlich zur bestehenden `CoreDataTrace.m`-Abwehr wird jetzt bereits in `frontend/ios/App/App/AppDelegate.swift` beim nativen Launch der Application-Support-Pfad inklusive `default.store`-Parent vorbereitet und mit **`[CoreDataFix]`**-Pfad-/State-Logs dokumentiert. `CoreDataTrace.m` loggt nun außerdem `before_store_parent_ensure`, `after_store_parent_ensure` und `store_file_before_open`. `frontend/scripts/ios-diagnose.js` und `frontend/ios/App/CORE_DATA_RUNTIME_TRACE.md` wurden auf diese neue Logspur erweitert. Verifiziert per **`yarn ios:diagnose` PASS**; echter Xcode-/iPhone-Laufzeittest bleibt weiterhin **User-Verifikation**.
- **18.07.2026 KYC temporär komplett für Tests deaktiviert**: Auf ausdrücklichen Nutzerwunsch ist KYC aktuell **testweise ausgestellt**. `frontend/.env` enthält nun **`REACT_APP_DISABLE_KYC=true`**. Der Schalter ist bewusst so gebaut, dass KYC später schnell wieder aktiviert werden kann, indem dieser Key wieder auf `false` gesetzt oder entfernt wird.
- **18.07.2026 Frontend-KYC-Gates zentral an Test-Schalter gehängt**: `frontend/src/utils/adminAccess.js` behandelt Nutzer bei aktivem Schalter als KYC-freigegeben; `frontend/src/app/pathUtils.js` blockiert keine KYC-Routen mehr; `frontend/src/App.js` umgeht Login-Redirects und KYC-Weiterleitungen; `frontend/src/components/KYCBanner.jsx` rendert nichts mehr; `frontend/src/pages/HomePage.jsx`, `MorePage.jsx`, `WalletPage.jsx` und `components/auctions/AuctionDetail.jsx` respektieren den Testmodus ebenfalls.
- **18.07.2026 Verifikation KYC-off**: Frontend neu gestartet, Smoke-Test auf Wallet erfolgreich ohne KYC-Banner, und **Testing Agent Iteration 262 PASS**. Bestätigt: keine KYC-Banner/Gates/Redirects mehr für normalen Reviewer-User; Wallet, More, Auktionen und Auktionsdetail bleiben erreichbar. **Wichtig:** Das ist ein absichtlicher temporärer Testmodus und soll laut Nutzer **erst wieder eingeschaltet werden, wenn ausdrücklich angefordert**.
- **18.07.2026 ReceiveMoney QR-Bug erneut sauber abgesichert**: Nach neuem Nutzer-Screenshot wurde bestätigt, dass der Fehler nicht nur fachlich im QR-Payload lag, sondern im nativen Build-Pfad zusätzlich durch **falsche Produktions-Backend-URL im iOS-Webbundle** wieder auftauchen konnte. `frontend/scripts/ios-prepare.js` liest jetzt `REACT_APP_BACKEND_URL` direkt aus `frontend/.env` und setzt sie vor `yarn build` + `npx cap sync/copy ios`, damit native Builds nicht versehentlich wieder `https://bidblitz.ae` aus `.env.production` einbacken, wenn eigentlich die aktuelle Preview-/Test-URL gemeint ist.
- **18.07.2026 ReceiveMoney Render-Fallback gehärtet**: `frontend/src/pages/ReceiveMoneyPage.jsx` enthält jetzt zusätzlich `ReceiveQrRenderBoundary`, damit selbst bei einem QR-Renderfehler innerhalb `QRCodeSVG` keine rohe Browser-/DOMException wie **`The string did not match the expected pattern.`** mehr sichtbar wird. Stattdessen greift ein kontrollierter deutscher Inline-Fallback.
- **18.07.2026 CoreDataTrace statisch nachgeschärft**: `frontend/ios/App/App/CoreDataTrace.m` enthält jetzt eine Forward-Declaration für `bb_trace_addPersistentStoreWithType...`, damit die in Xcode sichtbare Warnung **`Undeclared selector 'bb_trace_addPersistentStoreWithType...'`** beseitigt ist. Zusätzlich erstellt `BBEnsurePersistentStoreParentDirectory(...)` vorsorglich auch das Parent-Verzeichnis der Persistent-Store-URL.
- **18.07.2026 Verifikation dieses Bugfixes**: Preview-Smoke auf `/receive-money` zeigt QR wieder sichtbar; Build- und iOS-Bundle enthalten nun die **Preview-URL** statt hartem `bidblitz.ae`; **Testing Agent Iteration 260 PASS**. Statisch geprüft: CoreDataTrace-Patch vorhanden. **Wichtig:** echtes Laufzeitverhalten des CoreData-/SwiftData-Pfads bleibt weiterhin **User-/Xcode-Gerätetest**, da Linux-Container kein iPhone ausführen kann.
- **18.07.2026 Audi-Ticketverkauf mit Wallet-Zahlung live**: Die neue Seite `frontend/src/pages/AudiTicketSalesPage.jsx` ist unter **`/audi-tickets`** eingebunden und verkauft Audi-Eventtickets jetzt **direkt über das BidBlitz Wallet** statt über externen Checkout. Nutzer sehen öffentliche Eventdaten, Live-Bestände, Tickettypen, Gastfelder, persönliche Bestellübersicht und erhalten nach dem Kauf sofort digitale Ticketcodes. Admins sehen auf derselben Seite zusätzlich einen **Check-in-Block** für Vor-Ort-Validierung.
- **18.07.2026 Backend Audi-Tickets ergänzt**: Neue Route **`backend/routes/audi_tickets.py`** mit Auto-Seed für Event + Ticketkatalog, Endpunkten **`GET /api/audi-tickets/public/overview`**, **`GET /api/audi-tickets/my-orders`**, **`POST /api/audi-tickets/purchase`**, **`GET /api/audi-tickets/admin/dashboard`** und **`POST /api/audi-tickets/admin/checkin`**. Die Zahlung läuft korrekt über `transfer_between_wallets(...)` aus `core/payment_engine.py`, also **echte interne Wallet-Belastung + Plattform-Gutschrift** mit Referenz und Ledger-Spur.
- **18.07.2026 Audi-Ticket Datenmodell/Flow**: Collections/Objekte `audi_ticket_events`, `audi_ticket_types`, `audi_ticket_orders`, `audi_tickets`; Bestandslogik per atomarem `$expr`-Update gegen Race Conditions; Bestellungen enthalten `wallet_reference`, `wallet_transaction_id`, `ticket_codes`, `checkin_status`; einzelne Tickets speichern `qr_payload` und `checked_in_at`.
- **18.07.2026 Navigation/Discoverability erweitert**: `frontend/src/App.js` kennt jetzt **`/audi-tickets`**, `frontend/src/services/api.js` enthält die neuen Audi-API-Wrapper, `frontend/src/pages/AllServicesPage.jsx` zeigt **Audi Tickets** unter Buchen & Reservieren und `frontend/src/components/admin/sections.js` / `dataLoaders.js` kennen den Audi-Eintrag ebenfalls.
- **18.07.2026 Audi-Verifikation**: API-Self-Test PASS (Overview, Kauf, Orders, Admin-Dashboard, Check-in), Preview-Smoke PASS und **Testing Agent Iteration 259 PASS**. Report bestätigt **Backend 18/18 PASS** und **Frontend 100% PASS**. **Keine MOCKED APIs in diesem Audi-Block.** Einziger nicht-blockierender Hinweis bleibt das bekannte Cookie-Banner-Overlay unten rechts.
- **18.07.2026 SendMoney-Scanner inline auf iOS umgestellt**: `frontend/src/pages/SendMoneyPage.jsx` verwendet jetzt für native Capacitor-Umgebungen einen eingebetteten Scanner direkt in der Seite **„Empfänger-Code scannen“**. Dazu wurden **`@capgo/camera-preview@7.26.4`** (eingebettete Vorschau + Barcode-Events) und **`capacitor-native-settings@7.0.2`** (Button „Einstellungen öffnen“) integriert. Der Scanner startet jetzt automatisch beim Öffnen des Scan-Bereichs, fordert Kamera-Zugriff direkt an, zeigt die Vorschau innerhalb von `send-money-scanner-surface`, stoppt nach erfolgreichem QR-/BLZ-Scan, wechselt automatisch zum nächsten Schritt und räumt Listener/Kamera beim Verlassen sauber auf.
- **18.07.2026 Ursache des alten iOS-Problems dokumentiert**: Der bisherige Scan-Flow war für iOS im Kern ein Web-/Fallback-Ansatz (`html5-qrcode` + Bildaufnahme/Datei-Chooser) innerhalb eines separaten Scan-Sheets. Dadurch gab es keinen echten nativen eingebetteten Live-Scanner in der vorgesehenen Fläche; auf iPhones drohte stattdessen Kamera-/Capture-Fallback außerhalb des gewünschten In-App-Flows. Zusätzlich wurde ein Cleanup-Bug beseitigt: `scanner.stop()` konnte `undefined` zurückgeben, wodurch `stopCamera()` in manchen Fällen `Cannot read properties of undefined (reading 'catch')` auslöste.
- **18.07.2026 Verifikation Inline-Scanner**: `npm run build` PASS, `npx cap sync ios` PASS, Preview-Smoke PASS und **Testing Agent Iteration 258 PASS** mit **5/5** Frontend-Szenarien (Inline-Panel statt Overlay, sichtbare Scan-Fläche, Auto-Start, sauberer Close-Cleanup, Browser-Fallback). Ein echter physischer iOS-Kameratest bleibt mangels iPhone/Xcode-Ausführung im Linux-Container noch **User-Verifikation**.
- **18.07.2026 Wallet-Karten wieder tappbar**: In `frontend/src/pages/WalletPage.jsx` wurden die drei Info-Karten **„Kasse / Händler“**, **„Privat empfangen“** und **„Privat senden“** von statischen Anzeige-Elementen auf echte Buttons mit `onNavigate` umgestellt. Ursache des Mini-Bugs: die Karten waren reine `<div>`-Boxen ohne Click-/Tap-Handler. Jetzt führen sie direkt zu `/pay`, `/receive-money` und `/send-money`; Preview-Smoke bestätigt Navigation für Privat senden + Privat empfangen.
- **18.07.2026 ReceiveMoney-QR auf iOS korrigiert**: Die Seite `frontend/src/pages/ReceiveMoneyPage.jsx` (Texte: **„Empfangen“**, **„Mein QR-Code“**, **„Nur privat empfangen“**) rendert den QR-Code jetzt wieder robust. Statt `profile.qr_data` ungeprüft an `QRCodeSVG` zu geben, wird nun ein kontrollierter **JSON-String-Payload** aufgebaut: `type`, `userId`, `walletId`, `token`, `bidblitzId`, `name` und optional `amount`. Vor dem Rendern werden Pflichtwerte (`user.id`, `wallet.id`/`walletId`, `token`) validiert; wenn etwas fehlt, erscheint ausschließlich die deutsche Meldung **„Der QR-Code konnte noch nicht erstellt werden. Bitte erneut versuchen.“**. Die rohe technische Meldung **`The string did not match the expected pattern.`** wird nicht mehr an den Nutzer durchgereicht.
- **18.07.2026 Datenquelle vereinheitlicht**: `frontend/src/store/WalletContext.jsx` liefert jetzt explizit `id` und `walletId`. `backend/routes/p2p_transfer.py` erweitert `/api/p2p/qr/generate` um `user_id` und `wallet_id`. `backend/routes/wallet.py` erweitert `/api/wallet` um `id` und `wallet_id`. Dadurch kann die Receive-Seite User- und Wallet-Identität stabil zusammenführen, statt implizit auf unvollständige Context-/QR-Objekte zu vertrauen.
- **18.07.2026 Verifikation QR-Fix**: `npm run build` PASS, `npx cap sync ios` PASS, Preview-Smoke PASS und **Testing Agent Iteration 257 PASS** (5/5). Konsole zeigt jetzt einen validen String-Payload, z. B. `{"type":"p2p_receive","userId":"6a4f26f81e36bd715f730705","walletId":"6a4f26f81e36bd715f730705","token":"sWp1rQsswtgyVPJlakA6s-D07r-3aIiZppOadWCk0HM","bidblitzId":"BLZ-128B-8100","name":"BidBlitz Reviewer"}`. **Keine MOCKED APIs in diesem Fixblock.**
- **18.07.2026 iOS-Startup-Crash im React-Lazy-Chunk entschärft**: Der akute Frontend-Crash nach App-Start auf iOS wurde per Build-/Sourcemap-Analyse nicht auf Admin, sondern auf den **Wallet-Lazy-Chunk** zurückgeführt. `frontend/src/components/ErrorBoundary.jsx` wurde mit gemeinsamen Fehler-Helpern (`buildFrontendErrorPayload`, `postFrontendError`) gehärtet, sodass Frontend-Errors jetzt sauber **`message`**, **`stack`**, **`component_stack`**, **`page`** und strukturierte **`meta`**-Daten liefern statt leerem `{}`. `frontend/src/components/LazyErrorBoundary.jsx` nutzt dieselbe Payload-Logik.
- **18.07.2026 Wallet-Chunk iOS-sicher gemacht**: `frontend/src/pages/WalletPage.jsx` schützt jetzt alle kritischen **`window`**-, **`document`**- und **`navigator.clipboard`**-Zugriffe; das betrifft Stripe-Return-/Deep-Link-Pfade, Visibility-Events, Export-Popups, Confirm-Dialoge und Copy-Flows. `frontend/src/components/TopUpModal.jsx` schützt `window`-Zugriffe zusätzlich und entfernt den ungenutzten `AppleGooglePayButton`-Import aus dem Wallet-Chunk. `frontend/src/components/wallet/WalletSecurityCards.jsx` sichert `document.body`-Zugriffe ab. `frontend/src/hooks/usePayment.js` behandelt ungültige oder fehlende `transactions`-/Date-Werte robust, damit der Chunk nicht an unerwarteten Nutzdaten kippt.
- **18.07.2026 Verifikation**: Frontend-Build PASS, Preview-Smoke-Test PASS auf `/?stripe_session_id=test_ios_probe`, dazu **Testing Agent Iteration 256 PASS** mit **5/5 Frontend-Szenarien grün**: Stripe-Return-Pfad, direkte `/wallet`-Route, iPhone-Viewport, Mobile `/wallet` und authentifizierte Wallet-Ansicht. Ergebnis: **kein Blank Screen**, **keine sichtbare Error Boundary**, **keine leeren Error-Objekte**, **keine Chunk-Loading-Fehler**. **Keine MOCKED APIs in diesem Fixblock.**
- **CoreData-/SwiftData-Status**: Die native `default.store`-Directory-Abwehr über `CoreDataTrace.m` bleibt **User-Verifikation aus Xcode/iPhone ausstehend**. Der JavaScript-Startup-Blocker ist jedoch auf Preview-/Frontend-Ebene jetzt beseitigt.
- **Nächste fachliche Schritte**: Nach iOS-Gerätetest wieder zurück auf Produkt-Roadmap mit **Audi-Ticketverkauf auf der Webseite** und **Telegram-Alarm als Monitoring-Backup**.
- **16.07.2026 Auktions-Katalog-Sync finalisiert**: `backend/routes/auctions.py` synchronisiert jetzt nicht nur Detail- und Seed-Pfade, sondern auch die Listenendpunkte konsequent mit `resolve_product_gallery()`. Zusätzlich wird `ACTIVE_AUCTION_CATALOG` beim Laden direkt normalisiert, damit dynamisch erzeugte Auktionen dieselben modellgenauen `image_url`- und `image_urls`-Werte tragen wie Detailansichten. Verifiziert per API und UI-Smoke; Samsung und iPhone liefern nun auch im `/api/auctions/list` konsistente, unterschiedliche Primärbilder.
- **16.07.2026 Auktionsdetail-Bugs (Bild + Zeit) behoben**: Nach Nutzerhinweis wurden zwei konkrete UI-/Datenfehler im Auktionsmodul gefixt. **Bug 1 — Bild passt nicht zum Artikel:** In `backend/routes/auctions.py` überschrieben die Produkt-Fallback-Regeln vorhandene `image_url`/`image_urls` zu aggressiv. Dadurch konnte ein Artikel zwar einen korrekten gespeicherten Produktbild-Link haben, aber trotzdem mit einem generischen Kategorie-Fallback ausgeliefert werden. Der Fix priorisiert nun vorhandene Bilder strikt; Fallback greift nur noch, wenn wirklich kein brauchbarer Bildwert existiert. Zusätzlich reichert `GET /api/auctions/{auction_id}` das Detailobjekt jetzt selbst konsistent mit `image_url`, deduplizierten `image_urls`, `remaining_seconds` und `final_battle` an. **Bug 2 — verbleibende Zeit / Countdown kaputt:** `frontend/src/components/auctions/Countdown.jsx` zeigte bei Tagen/Stunden keine vollständigen Einheiten und war für die Detailseite visuell/inhaltlich unpräzise. Der Countdown zeigt nun auch bei längeren Laufzeiten **Tage + Stunden + Minuten + Sekunden** und behandelt ungültige Datumswerte defensiv. Parallel wurde `frontend/src/components/auctions/AuctionGridCard.jsx` so angepasst, dass das Grid-Label ebenfalls ein vollständigeres Format nutzt. Verifiziert wurde der Fix doppelt: Direkt per API (`/api/auctions/{id}` und `/api/auctions/list`) sowie per **Testing Agent Iteration 251 PASS**. Im Test wurden konkret iPhone-/Samsung-Smartphone-Auktionen und eine MacBook-Auktion geprüft; Bilder passen jetzt zum Produkttyp, und der Countdown läuft in Detail- und Grid-Ansicht wieder korrekt und sekündlich fort.
- **16.07.2026 Auktionsbeschreibung + modellgenaue Bilder weiter verfeinert**: Nach weiterem Nutzerfeedback („Beschreibung durcheinander“, „Fotos passen nicht zu Artikel“) wurde das Auktionsdetail nochmals gezielt nachgeschärft. Im Frontend rendert `frontend/src/components/auctions/AuctionDetail.jsx` die Beschreibung nun in kurzen, aufgeräumten Zeilen statt als potenziell unruhigen Freitextblock. Im Backend bekam `backend/routes/auctions.py` zusätzlich explizite Modell-Mappings in `resolve_product_image()`, damit zentrale Produkte nicht nur Kategorienbilder, sondern modellnähere Hero-Bilder erhalten. Abgedeckt wurden in diesem Schritt u. a. **iPhone 17 Pro Max 2026**, **Samsung Galaxy S26 Ultra Elite 2026**, **Google Pixel 10 Pro 2026**, **Xiaomi 16 Ultra 2026**, **MacBook Pro M6 Max 16 2026**, **Dell XPS 17 AI 2026**, **Lenovo Yoga Pro 9i Aura 2026** und **ASUS ROG Zephyrus G16 2026**. Verifiziert per **Testing Agent Iteration 253 PASS**: Beschreibung bei iPhone, Samsung und MacBook ist jeweils sauber/einzeilig, Bilder passen zum konkreten Modellspektrum, Galerie bleibt bei 4 klickbaren Bildern, Countdown ohne Regression.
- **16.07.2026 Samsung/iPhone-Fotoverwechslung an der eigentlichen Quelle behoben**: Nach weiterem sehr konkretem Nutzerfeedback („Samsung im Text, iPhone-Foto im Bild“) wurde die Bildlogik nochmals bis zum Datenursprung korrigiert. Die Ursache war, dass `resolve_product_gallery()` zuvor für mehrere Smartphone-Modelle dieselbe Smartphone-Sammelgalerie in derselben Reihenfolge nutzte. Dadurch konnte das Primärbild zwar bereits besser sein, aber die Gallery-Reihenfolge blieb nicht artikelgenau genug. Der Fix trennt jetzt die Gallery-Reihenfolge explizit pro Modellfamilie: **iPhone 17 Pro Max**, **Samsung Galaxy S26 Ultra Elite**, **Google Pixel 10 Pro**, **Xiaomi 16 Ultra** sowie zentrale Laptop-Modelle bekommen jeweils eine eigene geordnete Galerie. Gleichzeitig wurden alle relevanten Erzeugungspfade (`_build_auction_doc`, Einzel-Schedule, Bulk-Schedule) so angepasst, dass neue/erneuerte Auktionen diese artikelgenauen `image_urls` direkt persistiert bekommen. Final verifiziert per **Testing Agent Iteration 254 PASS**: iPhone nutzt nun sichtbar Primärbild `photo-1517777298614`, Samsung Primärbild `photo-1544866092` (**explizit verschieden vom iPhone**), MacBook Primärbild `photo-1511385348`; alle drei getesteten Detailseiten zeigen **4 Galerie-Bilder** und weiterhin passende, saubere Beschreibungen. Damit ist der von dir beschriebene Samsung/iPhone-Mismatch jetzt nicht nur kosmetisch, sondern **an der eigentlichen Datenquelle** behoben.
- **15.07.2026 Schwimmbad-System MVP neu aufgebaut**: Auf neue Produktanforderung wurde ein eigenständiges **Schwimmbad-/Freizeitbad-System** als vollwertiges Modul im bestehenden BidBlitz-Superapp-Stack ergänzt. Ziel laut Nutzerwahl: **komplettes MVP**, **alle Kernfunktionen**, **echtes Web-System**, **Bar/Kasse vor Ort plus Online-Zahlung**, **Deutsch + Englisch** und **Pool-/Freizeitbad-Look** gemäß Referenzbildern. Umgesetzt wurde eine öffentliche Seite **`/pool`** (`frontend/src/pages/PoolFacilityPage.jsx`) mit Ticketpaketen, Extras (Spind, Sauna, Liege), Gastdaten, Live-Auslastung, Online-Checkout und digitalem Ticketstatus. Parallel entstand ein Betreiberbereich **`/admin/pool`** (`frontend/src/pages/PoolAdminPage.jsx`) mit Tabs für **Übersicht**, **Kasse**, **Einlass**, **Spinde**, **Snack POS** und **History**. Backend-seitig wurde dazu **`backend/routes/pool_management.py`** eingeführt und in `core/router_registry.py` registriert. Das Modul liefert Endpunkte für Public Overview, Stripe-Checkout, Checkout-Status, Betreiber-Dashboard, Kassenverkauf, Locker-Assign/Release, Turnstile-Scan und Snack-Verkäufe. Für Online-Zahlungen wird die vorhandene Stripe-Infrastruktur wiederverwendet; `routes/stripe.py` ruft bei bezahlter Session jetzt zusätzlich `handle_pool_ticket_webhook()` auf, wodurch aus `payment_transactions` des Typs `pool_ticket` echte Pool-Tickets entstehen. Fachlich enthält das MVP: Ticket-/Tarifstruktur, QR-/RFID-Armband-Readiness, Spindzuweisung, Einlass-/Auslass-Logik und Snack-POS. **WICHTIG:** Reale Hardware-Anbindungen sind in diesem Block klar als **MOCKED** markiert und betreffen **RFID bridge**, **Turnstile bridge** und **Locker relay bridge** — die Softwarelogik ist live, Hardware-Treiber/Provider aber noch nicht integriert. Navigation/Entdeckbarkeit wurden ebenfalls ergänzt: `App.js` kennt jetzt `/pool` und `/admin/pool`, `AdminPage.jsx` und `components/admin/sections.js` verlinken den Betreiberbereich, `AllServicesPage.jsx` zeigt „Schwimmbad“ unter Buchen & Reservieren, und `services/api.js` enthält die passenden Pool-API-Wrapper. Während der Implementierung gab es wiederholt Infrastrukturgrenzen durch **`No space left on device`** auf `/app`; deswegen wurde der initiale Locker-Seed im MVP bewusst klein gehalten (**8 Lockers** statt voller Anlage), damit das Modul in dieser Umgebung stabil bleibt. Verifiziert wurde der Block doppelt: **Backend-Self-Test PASS** (`overview`, `dashboard`, `cash sale`, `locker assign`, `turnstile entry`, `snack sale`) sowie **Testing Agent Iteration 250 PASS** mit **7/7 Kernfeatures grün** und **41 data-testid** bestätigt. Einziger nicht-blockierender Hinweis des Testings: Cookie-Banner überlagert unten rechts einen kleinen Bereich, blockiert aber keine Kernfunktion.
- **15.07.2026 Hardware-Integrationsarchitektur für Pool-System definiert**: Im Anschluss wurde die gewünschte echte Hardware-Anbindung zunächst **vendor-neutral fachlich definiert**, ohne zu früh auf einen Hersteller festzulegen. Der Nutzerwunsch war: **NFC/MIFARE und QR-Armbänder**, **REST/HTTP sowie TCP/Serial für Drehkreuze**, **Netzwerk-API sowie Relay-Bridge für Spinde** sowie Unterstützung für **Cloud**, **lokalen Edge-Dienst** und **Hybrid/Gateway-Box**. Dafür wurde `backend/routes/pool_management.py` um ein Hardware-Blueprint erweitert (`HARDWARE_BLUEPRINT`, `DEFAULT_HARDWARE_CONFIG`) und zwei neue Betreiber-Endpunkte ergänzt: **`GET /api/pool/admin/hardware/config`** und **`POST /api/pool/admin/hardware/config`**. Persistiert werden diese Einstellungen in `pool_hardware_config`; Änderungen schreiben gleichzeitig ein einfaches Audit-/Event-Log in `pool_hardware_events`. Frontend-seitig bekam `frontend/src/pages/PoolAdminPage.jsx` den neuen Tab **Hardware**, in dem jetzt Deployment-Modus, RFID-Provider-Modus, Reader-/Turnstile-/Locker-Adaptertyp, Shared-Secret-Hinweis, Architektur-Karten, Adapter-Feldlisten und das Hardware-Event-Log sichtbar und änderbar sind. Damit kann das Team die Zielarchitektur pro Bad/Standort schon im Produkt pflegen, bevor ein konkreter Reader-/Controller-Hersteller feststeht. Live verifiziert per Admin-Login und API: `GET /api/pool/admin/hardware/config` => **200**, `POST /api/pool/admin/hardware/config` => **200 / ok=true**, gespeicherter Zielmodus testweise auf **`hybrid_gateway`** mit **`serial_reader_bridge`**, **`serial_turnstile_bridge`** und **`relay_locker_bridge`** gesetzt; anschließend zeigte `GET /api/pool/admin/dashboard` die aktualisierte Konfiguration plus **1 Hardware-Event**. **Wichtig:** Auch nach diesem Schritt bleiben reale Herstelleradapter/SDKs weiterhin **MOCKED** – jetzt ist jedoch die operative Vertrags- und Datenstruktur für die nächste Integrationsphase vorbereitet.
- **15.07.2026 RTK CLI Proxy im Container installiert/verifiziert**: Auf direkten Nutzerwunsch wurde **RTK v0.43.0** im aktuellen **aarch64 Kubernetes-Container** installiert. Der ursprünglich bevorzugte Weg über das GitHub-Release-Binary (`rtk-aarch64-unknown-linux-gnu.tar.gz`) war in dieser Umgebung **nicht direkt lauffähig**, da das Binary `GLIBC_2.39` verlangte, der Container aber **GLIBC 2.36** bereitstellt. Statt in einer Binary-Schleife hängen zu bleiben, wurde RTK sauber aus dem offiziellen GitHub-Tag **`v0.43.0`** via Rust/Cargo lokal kompiliert und nach `/usr/local/bin/rtk` installiert. Danach wurde der gewünschte globale Minimal-Setup aktiviert: `rtk init -g --hook-only --auto-patch`, wodurch `~/.claude/settings.json` jetzt den Hook **`rtk hook claude`** für Bash-PreToolUse enthält. Zusätzlich wurde `rtk telemetry forget` ausgeführt und `/root/.config/rtk/config.toml` geprüft, damit Telemetry deaktiviert bleibt. Verifiziert per Bash mit `rtk --version`, `rtk init --show`, `rtk git status`, `rtk ls /app` und `rtk gain`; letztere zeigte direkt erste Nutzung mit **2 Commands** und rund **58% Token-Ersparnis**. **Wichtig:** Das ist ein **Tooling-/Container-Setup** und **keine neue App-User-Feature-API**; Frontend-/Backend-Kernflows der Super-App wurden dabei nicht verändert.
- **15.07.2026 RTK Balanced-Konfiguration + Multi-Agent-Rollout ergänzt**: Im nächsten Schritt wurde RTK nicht nur „installiert“, sondern auf **praktische Default-Regeln** für diesen Container gebracht. In `/root/.config/rtk/config.toml` wurde eine gezielte **`include_commands`-Liste** für typische Entwicklungs-/Infra-Kommandos gesetzt (u. a. `git`, `ls`, `find`, `grep`, `pytest`, `cargo`, `docker`, `kubectl`, `pnpm`, `npm`, `tsc`, `eslint`, `prisma`, `mvn`, `pip`, `uv`). Parallel wurden bewusst sensible oder ungeeignete Auto-Rewrites über **`exclude_commands`** ausgespart, darunter `curl`, `wget`, `playwright`, `ssh`, `scp`, `tar`, `zip`, `openssl`, `docker run`, `docker compose up`, `kubectl apply` und `kubectl create`. Zusätzlich wurde RTK auf weitere im Container sinnvolle Agent-Modi ausgerollt: **Codex** (`/root/.codex/AGENTS.md`, `/root/.codex/RTK.md`), **Gemini CLI** (`/root/.gemini/settings.json`, Hook-Skript + `GEMINI.md`), **Hermes** (`/root/.hermes/config.yaml`, Plugin `rtk-rewrite`) sowie **Cursor** (`/root/.cursor/hooks.json`). Der Claude-Teil wurde dabei von reinem Hook-Only auf den vollständigen globalen Modus erweitert (`~/.claude/RTK.md` und `~/.claude/CLAUDE.md` zusätzlich vorhanden), der Hook selbst blieb aktiv. Verifiziert durch Datei-Checks, `rtk init --show` und `rtk rewrite`-Kontrollen: `git status` und `pytest -q` werden weiter zu RTK umgeschrieben, während `curl` oder `docker run` absichtlich **nicht** rewritet werden. Telemetry wurde danach erneut mit `rtk telemetry forget` deaktiviert. Auch dieser Block ist **reines Tooling-/Container-Setup** und berührt keine produktiven User-Flows der App.
- **15.07.2026 RTK Admin Dashboard eingebaut**: Damit RTK nicht nur „im Container existiert“, sondern im Produktkontext sichtbar/debuggbar ist, wurde eine neue Admin-Seite **`/admin/rtk`** ergänzt (`frontend/src/pages/AdminRtkPage.jsx`). Diese Seite zeigt live den RTK-Binary-Status, Installationsquelle, Telemetry-Status, Config-Pfad, Include/Exclude-Regeln, Agent-Rollouts für Claude/Cursor/Codex/Gemini/Hermes, Rewrite-Beispiele und Savings-Kennzahlen. Backend-seitig wurde dazu die abgesicherte Admin-Route **`GET /api/diag/rtk`** in `backend/routes/diag.py` ergänzt. Die Route liest ausschließlich tool-nahe RTK-Metadaten (Binary/Version, Config über `tomllib`, Hook-Dateien, `rtk gain --all --format json`, `rtk rewrite` für sichere Beispielkommandos) und gibt eine JSON-sichere Diagnose zurück. Frontend-seitig wurde die neue Seite in `App.js`, `AdminPage.jsx`, `components/admin/sections.js`, `components/admin/dataLoaders.js`, `components/admin/AdminDetailRouter.jsx` und `adminRouteMap.js` eingehängt, sodass RTK jetzt direkt aus dem Admin-Systembereich erreichbar ist. Verifiziert per Admin-Login + `/api/diag/rtk` End-to-End: **RTK 0.43.0**, **5/5 Agent-Setups aktiv**, **30 Include-Regeln**, **20 Exclude-Regeln**, `git status`/`pytest -q` werden als Rewrite erkannt, `curl`/`docker run` bleiben Passthrough. Der geplante visuelle Screenshot-Smoke scheiterte in diesem Fork nur an einem **Container-Disk-Space-Problem** (`No space left on device` im Automations-Output-Verzeichnis), nicht an der neuen RTK-Seite selbst.
- **15.07.2026 Repo-spezifische RTK-Filter ergänzt**: Im nächsten Schritt wurde RTK auch projektspezifisch auf dieses BidBlitz-Monorepo zugeschnitten. Dazu wurde eine neue Datei **`/app/.rtk/filters.toml`** angelegt, die aktuell **6 lokale Filter** enthält: `vitest-noise`, `pytest-brief`, `grep-clean`, `git-status-focus`, `supervisor-tail-focus` und `curl-json-noise`. Diese Regeln sind bewusst auf typische Arbeitsmuster in diesem Repository ausgerichtet: Rauschen von Vite/Vitest-Devservern reduzieren, `pytest`-Header/Plugin-Noise kürzen, `grep`/`rg`-Treffer begrenzen, `git status` fokussieren, Supervisor-Logs auf die letzten relevanten Zeilen eindampfen und JSON-Antworten aus Diag-/Auth-/Flag-Endpunkten leichter lesbar machen. Parallel wurde `backend/routes/diag.py` erweitert, damit `/api/diag/rtk` jetzt nicht nur globale RTK-Config und Agent-Hooks, sondern auch **Projektfilter-Metadaten** liefert: Existenz, Schema-Version, Filternamen, Filteranzahl, Trust-Store-Pfad und ob die Repo-Filter bereits als **trusted** gelten. Im Frontend zeigt `frontend/src/pages/AdminRtkPage.jsx` dafür nun eine eigene Projektfilter-Sektion mit Trust-Hinweis und Filter-Chips an. Verifiziert per Bash/API: `.rtk/filters.toml` existiert sauber, die API meldet **6 lokale Filter** mit Namen korrekt zurück und markiert sie aktuell absichtlich als **nicht trusted**, bis im Projektverzeichnis bewusst `rtk trust` ausgeführt wird. Dieser Schritt verändert erneut **nur Tooling-/Agent-Verhalten**, keine produktive Endkunden-Funktion der App.
- **15.07.2026 RTK-Adminaktionen + Live-Trust aktiviert**: Anschließend wurde die RTK-Verwaltung in einen wirklich bedienbaren Admin-Flow erweitert. Frontend-seitig erhielt `frontend/src/pages/AdminRtkPage.jsx` einen neuen **Admin-Aktionsblock** mit direkten Buttons für: **Projektfilter trusten**, **Telemetry erneut deaktivieren** und **Agent-Dateien neu generieren**. Alle Buttons, Zustände und Ergebnisboxen wurden mit `data-testid` versehen. Backend-seitig wurden dazu die abgesicherten Endpunkte **`POST /api/diag/rtk/trust-project-filters`**, **`POST /api/diag/rtk/telemetry/forget`** und **`POST /api/diag/rtk/reapply-agents`** ergänzt. Jede Aktion führt die zugehörigen RTK-Kommandos serverseitig kontrolliert aus und gibt neben dem Laufresultat sofort die aktualisierte RTK-Status-Payload zurück, sodass das Dashboard direkt ohne manuellen Refresh reagieren kann. Im Zuge dessen wurde außerdem ein bereits abgeschnittener Fehler am Dateiende von `backend/routes/diag.py` repariert (`migration_audit_log_rollback`), der zwischenzeitlich die `diag`-Route-Registrierung gebrochen hatte. Nach Fix und Reload war der Server wieder sauber bei **194 registrierten Routern**. Live verifiziert per Admin-Login und API: `trust-project-filters` => **200 / ok=true**, `telemetry/forget` => **200 / ok=true**, `reapply-agents` => **200 / ok=true**. Danach meldet `/api/diag/rtk` nun **`project_filters.trusted = true`**, **`telemetry_enabled = false`** und weiterhin **5 aktive Agent-Setups**. Damit ist der RTK-Adminbereich jetzt nicht nur rein lesend, sondern bereits für zentrale sichere Wartungsaktionen nutzbar.
- **15.07.2026 Rewrite-Test live aus dem RTK-Dashboard ausführbar**: Im nächsten Schritt wurde der RTK-Adminbereich noch um eine vierte sichere Aktion erweitert: **„Rewrite-Test neu ausführen“**. Backend-seitig wurde dafür **`POST /api/diag/rtk/rerun-rewrite-tests`** ergänzt. Diese Aktion ist rein diagnostisch: Sie baut die RTK-Status-Payload frisch auf und gibt zusätzlich eine kleine Summary der Rewrite-Samples zurück (`rewritten`, `passthrough`, `total`). Frontend-seitig erhielt `frontend/src/pages/AdminRtkPage.jsx` den neuen Button inklusive `data-testid`, sodass der aktuelle Rewrite-Zustand mit einem Klick ohne Shell-Zugang neu geprüft werden kann. Live verifiziert per Admin-Login + API: **200 / ok=true**, Summary **2 rewritten / 2 passthrough / 4 total**. Damit deckt `/admin/rtk` jetzt die vier zentralen Betriebsaktionen ab: Trust, Telemetry-Forget, Agent-Reapply und Rewrite-Recheck.
- **15.07.2026 RTK-Event-History ergänzt**: Abschließend wurde `/admin/rtk` noch um eine sichtbare Verlaufsansicht erweitert. Backend-seitig schreibt `backend/routes/diag.py` jetzt jede RTK-Adminaktion in ein leichtgewichtiges JSONL-Log unter **`/root/.local/share/rtk/admin_actions.jsonl`**. Pro Eintrag werden Aktion, Erfolg/Fehler, Nachricht, UTC-Zeitstempel sowie gekürzte stdout/stderr-Summaries gespeichert. Gleichzeitig wurde die RTK-Status-Payload um **`action_history`** ergänzt, sodass das Dashboard beim normalen Status-Load die letzten Aktionen direkt mitbekommt. Frontend-seitig zeigt `frontend/src/pages/AdminRtkPage.jsx` diese History als eigene Karte mit Zeitstempeln, Status-Badges und Kurzresultaten an. Live verifiziert nach einer erneuten `telemetry_forget`-Aktion: Die Logdatei wurde angelegt, `GET /api/diag/rtk` liefert die History, und der letzte Eintrag erscheint mit `action = telemetry_forget`, `ok = true` und dem gekürzten RTK-Output. Damit ist der RTK-Adminbereich jetzt nicht nur bedienbar, sondern auch nachvollziehbar auditierbar.
- **15.07.2026 Project Filter Diff / Validation ergänzt**: Im nächsten Schritt wurde die Repo-Filter-Verwaltung noch um echte Strukturprüfung und Vergleichbarkeit erweitert. Backend-seitig liefert `/api/diag/rtk` jetzt zusätzlich **`project_filters_validation`** mit Validitätsstatus, Fehlern, Warnungen, Signaturdaten und Diff zur zuletzt gespeicherten Baseline (`added`, `removed`, `changed`). Dafür wurden die Endpunkte **`POST /api/diag/rtk/validate-project-filters`** und **`POST /api/diag/rtk/project-filters/save-baseline`** ergänzt. Die Baseline wird als JSON unter **`/root/.local/share/rtk/project_filters_baseline.json`** gespeichert und enthält Schema-Version plus normalisierte Filter-Signaturen. Frontend-seitig zeigt `frontend/src/pages/AdminRtkPage.jsx` im Projektfilter-Bereich jetzt Validierungsstatus, Diff-Zähler und Fehler-/Warnungslisten; im Aktionsblock kamen zusätzlich die Buttons **„Filter validieren“** und **„Baseline speichern“** hinzu. Live verifiziert per Admin-Login + API: erste Validierung **200 / ok=true / valid=true** bei noch fehlender Baseline mit allen 6 Filtern als `added`; nach Speichern der Baseline erneut **200 / ok=true**, danach meldet `/api/diag/rtk` konsistent **`baseline_exists = true`** und ein leeres Diff (`added=[]`, `removed=[]`, `changed=[]`). Damit ist die RTK-Repo-Filter-Verwaltung nun nicht nur sichtbar, sondern auch strukturell prüfbar und versionsvergleichbar.
- **13.07.2026 P0 Albanisch-Sweep für tiefe POS-/Merchant-Subtabs abgeschlossen**: Nach Anschlussauftrag wurden die bisher verbleibenden tieferen POS-/Merchant-Bereiche sprachlich bereinigt. Sichtbare Kerntexte, Buttons, Empty-States und Einleitungen wurden in den tiefen Screens auf Albanisch-/Mehrsprachenlogik gezogen in: `frontend/src/pages/POSAdvancedTab.jsx`, `POSProTab.jsx`, `POSRestaurantTab.jsx`, `POSExtendedPage.jsx` und `MerchantAdminPage.jsx`. Zusätzlich wurden mehrere Hook-/Lint-Probleme in diesen tiefen Tabs bereinigt (u. a. leere `catch`-Blöcke, stabile `useCallback`-Reloads, saubere `data-testid`-/Visible-State-Pfade). Verifiziert mit `mcp_lint_javascript` pro Datei und finalem `auto_frontend_testing_agent` PASS: **/pos/extended**, **/pos/advanced**, **/pos/pro**, **/pos/restaurant** und **/admin/merchants** zeigen in aktivem Albanisch **keine deutschen Resttexte** mehr. **Keine MOCKED APIs in diesem Sweep.**
- **13.07.2026 P0 Scanner + Privat-Senden + Albanisch-Fixes abgeschlossen**: Nach Nutzer-Meldung wurden drei zusammenhängende P0-Bugs im Wallet-/Merchant-/POS-Bereich behoben. **(1) Privat-Senden / Scanner:** `frontend/src/pages/SendMoneyPage.jsx` nutzt jetzt für den privaten Scanner robustere Barcode-/QR-Erkennung (mehrere Barcode-Formate statt nur QR, stabilerer HTML5-Kamera-Fallback, Fokus-/Kamera-Setup näher am stabilen Hauptscanner) und behandelt **private BLZ-Empfänger-Codes** korrekt. Zusätzlich wurde der manuelle Scanner-Pfad für private BLZ-Codes verifiziert; Lookup gegen `admin@bidblitz.ae` liefert `BLZ-5824-A240`, und derselbe BLZ-Code führt im Scanner-Sheet korrekt in den privaten Sendeflow. **Zusatz-Check Kamera/Foto-Scan:** Der Upload-/Foto-Scanpfad wurde mit einem echten generierten QR-Bild (`BLZ-5824-A240`) erfolgreich Ende-zu-Ende verifiziert; zusätzlich wurde nachgeschärft, dass bei Bild-Scans nach einem `Html5Qrcode.scanFile`-Fehlschlag noch ein zweiter `BarcodeDetector`-Fallback versucht wird. Ein synthetischer **Code128-SVG-Test** im Headless-Browser blieb weiterhin ohne Treffer, aber der relevante QR-/BLZ-Foto-Scanpfad für private Sendungen funktioniert produktiv im Browser-Test. **(2) Privat-Senden funktional verifiziert:** echter UI-Test mit Reviewer-Konto (`reviewer@bidblitz.ae`) → Empfängersuche `admin@bidblitz.ae` → erfolgreiche Testüberweisung `0.01 EUR` → Albanisch lokalisierter Success-State. **(3) Übersetzungen / Albanisch:** sichtbare Kerntexte in `SendMoneyPage.jsx`, `MerchantPage.jsx`, `MerchantConnectPage.jsx`, `POSPage.jsx`, `ReceiveMoneyPage.jsx` sowie im Home-Mining-Trust-Hero wurden auf echtes Sprachverhalten gezogen, inklusive Scanner-Sheet, Merchant-Stripe-CTA, POS-Onboarding-Hülle, Receive-QR-Screen und Mining-Trust-Hero. Zusätzlich bekommt `/pos` jetzt auch im Loading-/Onboarding-Zustand ein stabiles `data-testid="pos-page"`, damit Route und Onboarding sauber testbar bleiben. **(4) Senden↔Empfangen Paarflow verifiziert:** als Admin wurde ein echter `/api/p2p/qr/generate`-QR erzeugt, lokal als Testbild gerendert und im Reviewer-Flow über `/send-money` eingescannt. Ergebnis: Gegen-Scan führt sauber zu **BidBlitz Admin / BLZ-5824-A240** in den privaten Sende-Flow. Verifiziert via mehrfache Browser-Smokes, `auto_frontend_testing_agent` **Iteration 160 + 161 PASS** plus zusätzlicher P2P-Peer-Flow PASS und `deep_testing_backend_v2` PASS: Login, `PUT /api/user/profile` (`sq-XK`), `POST /api/p2p/lookup` per E-Mail und per BLZ-Code sowie privater BLZ-/Receive-QR-Sendeflow sind grün. **Keine MOCKED APIs in diesem Fixblock.**
- **12.07.2026 Bulk Child→User + Personen-Merge-Ansicht ergänzt**: Auf User-Wunsch wurden **A + B zusammen** umgesetzt. **A (Bulk Child→User):** Starke Child-/Backup-Fälle können jetzt gesammelt in echte Login-User überführt werden. Backend neu in `backend/routes/admin_legacy_restore.py`: `ChildToUserBulkPreviewRequest`, `ChildToUserBulkConfirmRequest`, `_suggest_child_restore_email()`, `_build_child_to_user_bulk_preview()`, `POST /api/admin/legacy-restore/child-to-user/bulk-preview`, `POST /api/admin/legacy-restore/child-to-user/bulk-confirm`. Das System generiert deterministische Vorschlags-E-Mails je Kandidat und Domain (z. B. `anuar.childa@restore.bidblitz.local`) und trennt wieder in `restoreable` vs. `blocked`. Verifiziert: Bulk-Vorschau für **Anuar**, **Emma** und **Albin** liefert 3 restorebare Kandidaten. **B (Personen-Merge-Ansicht):** Child-, Wallet-, Alias- und aktive Login-Spuren werden jetzt zu Personenclustern zusammengeführt. Backend neu: `_derive_person_cluster_key()`, `_build_person_merge_clusters()`, `GET /api/admin/legacy-restore/person-merge/overview`, `GET /api/admin/legacy-restore/person-merge/{cluster_id}`. Frontend neu im `LegacyRestoreCenterTab`: eigener Block **„Personen-Merge-Ansicht“** mit Summary-Karten, Clusterliste und Detailpanel. Beispiel **Albin**: Merge-Detail zeigt korrekt **3 Mitglieder** in einem Cluster — (1) Child-Kandidat `child:child_2a880974de5f`, (2) Screenshot-/Restore-Kandidat `albinkrasniqi11@icloud.com`, (3) aktiver User `albinkrasniqi11@icloud.com` inkl. Alias `albinkrasniqi612@gmail.com`. Gesamt-Clusterstand aktuell: **30 Cluster**, davon **6 mit aktivem User**, **6 mit Child-Spuren**, **3 mit möglichen echten Produktivfällen**. Verifiziert via `testing_agent` **Iteration 247 PASS**: 24/24 Backend-Tests grün, beide neuen UI-Blöcke vollständig vorhanden, keine Regression in Review-Enrichment, Standard-Bulk-Restore, Child-Single-Preview, Filtern oder History. **Keine MOCKED APIs in diesem Feature.**
- **12.07.2026 Child→User Assistent ergänzt**: Nach erfolgreicher Review-Anreicherung wurde im Legacy Restore Center ein kontrollierter **Child→User Assistent** ergänzt, um starke Child-/Backup-Fälle in echte Login-Nutzer zu überführen. Hintergrund: Fälle wie **Albin**, **Anuar** und **Emma** hatten genug Zusatzspuren, aber noch keine belastbare Login-E-Mail. Backend neu in `backend/routes/admin_legacy_restore.py`: `ChildToUserPreviewRequest`, `ChildToUserConfirmRequest`, `_build_child_to_user_preview()`, `POST /api/admin/legacy-restore/child-to-user/preview`, `POST /api/admin/legacy-restore/child-to-user/confirm`. Der Preview-Flow prüft: (1) Kandidat muss `possible_real_customer` sein, (2) Admin muss eine neue Login-E-Mail liefern, (3) Kollisionen mit bestehenden aktiven/restaurierten Accounts werden blockiert. UI neu in `LegacyRestoreCenterTab.jsx`: Panel **„Child → User Assistent“** im Detailbereich für angereicherte Review-Kandidaten. Enthalten: E-Mail-Feld, Namensfeld, Alias-Feld, Salden, Vorschau, temporäres Passwort und Bestätigungsbutton. Verifiziert via `testing_agent` **Iteration 246 PASS**: **Anuar** Preview mit `anuar.child.restore@bidblitz.com` → `restore_ready=true`, **Emma** Preview mit `emma.child.restore@bidblitz.com` → `restore_ready=true`, **Albin** mit bestehender Alias-/Login-E-Mail (`albinkrasniqi11@icloud.com` oder `albinkrasniqi612@gmail.com`) → korrekt blockiert mit `existing_user`. Keine Regression in Review-Enrichment, Filtern, Bulk Restore oder History. **Keine MOCKED APIs in diesem Feature.**
- **12.07.2026 Review-Fälle automatisch angereichert**: Als nächster Ausbau des Legacy Restore Centers wurde eine echte **Review-Anreicherung** ergänzt, damit Child-/Backup-Spuren nicht nur passiv gelistet, sondern gegen weitere Backup-Collections automatisch verdichtet werden. Backend neu in `backend/routes/admin_legacy_restore.py`: `CHILD_SIGNAL_CONFIG` definiert 6 Quellen (`child_alerts.json`, `kids_notifications.json`, `kids_tasks.json`, `kids_location_history.json`, `kids_zones.json`, `app_rules.json`). `_build_child_only_signals()` ergänzt Child-Kandidaten jetzt mit zusätzlichen Evidenzen (`child_signal_count`). `_classify_candidate()` stuft Child-Fälle mit **>=2 Zusatzspuren** automatisch von `review_required` zu **`possible_real_customer`** hoch. Neuer Endpoint: `POST /api/admin/legacy-restore/review-enrichment` liefert `review_visible`, `enriched_review_candidates`, `upgrade_ready_candidates` plus Liste der angereicherten Fälle. Frontend neu: im `LegacyRestoreCenterTab` wurde das Panel **„Review-Fälle automatisch anreichern“** mit Action-Button, Ergebnis-Summary und Kandidatenliste ergänzt. Ergebnis aktuell: **3 angereicherte Review-Fälle** wurden als `possible_real_customer` identifiziert — **Albin** (`child_2a880974de5f`, **6 Spuren**), **Anuar** (`child_a240c029dd30`, **5 Spuren**) und **Emma** (`child_888c787e77d9`, **2 Spuren**). Diese Fälle erscheinen jetzt als viel stärkere Restore-Kandidaten, obwohl weiterhin keine belastbare Login-E-Mail für einen finalen echten User-Restore vorliegt. Verifiziert via `testing_agent` **Iteration 245 PASS**: 17/17 Backend-Tests grün, UI-Panel + Button + Ergebnisliste funktionsfähig, bestehende Filter/Prioritäten/Bulk-Restore ohne Regression. **Keine MOCKED APIs in diesem Feature.**
- **12.07.2026 Echter-Kunden-Filter im Legacy Restore Center ergänzt**: Auf User-Wunsch wurde die Kandidatenliste jetzt so gefiltert, dass standardmäßig **nur echte produktive Alt-Kundenfälle** sichtbar bleiben. Backend in `backend/routes/admin_legacy_restore.py` erweitert um Klassifizierung und View-Filter: `_classify_candidate()` ordnet jede Spur einer Kategorie zu (**`real_customer`**, **`possible_real_customer`**, **`review_required`**, **`synthetic_test`**, **`attack_trace`**). `_apply_view_filter()` steuert die Ansichten `real_only`, `review`, `noise_only`, `all`. Damit werden echte Screenshot-/Wallet-Kandidaten von Test-/QA-/Lockout-/Bruteforce-Spuren getrennt. Verwendete Heuristiken: bekannte Screenshot-Seeds = echte Kunden, Child-only-Backups = Review, Domains/Pattern wie `example.com`, `test.com`, `bruteforce`, `lockout`, `iter`, `invalid` = synthetisch/Angriff. `GET /api/admin/legacy-restore/overview` liefert jetzt zusätzliche Summary-Felder: `visible_candidates`, `hidden_candidates`, `real_customer_candidates`, `review_candidates`, `synthetic_candidates`, `attack_candidates`, `view_mode`. Frontend in `LegacyRestoreCenterTab.jsx` ergänzt um Filterbuttons **Nur echte Kunden / Nur Review-Fälle / Nur Test/Attacke / Alle Spuren**, Kategorie-Badges und Filter-Summary. Ergebnis: Default `real_only` zeigt jetzt **nur 5 echte Kundenfälle** (Afrim Krasniqi, Afrim Test Final, Albin Krasniqi, Aldin Krasniqi, Test GmbH) und blendet 46 technische oder unklare Spuren aus. `review` zeigt 9 Review-Kandidaten, `noise_only` zeigt 37 synthetische/Attack-Spuren, `all` zeigt alle 51 Kandidaten. Verifiziert via `testing_agent` **Iteration 244 PASS**: 12/12 Backend-Tests grün, Filterumschaltung funktioniert korrekt, Priority List + Bulk Restore bleiben intakt. **Keine MOCKED APIs in diesem Feature.**
- **12.07.2026 Prioritätenliste + Bulk Restore + weitere Altkonten abgeschlossen**: Auf User-Wunsch wurden **C + A + B zusammen** umgesetzt. **C (Prioritätenliste):** Das Legacy Restore Center liefert jetzt in `GET /api/admin/legacy-restore/overview` zwei priorisierte Bereiche: `top_candidates` (sichere Screenshot-/Wallet-Treffer, auch wenn bereits repariert) und `top_missing_candidates` (noch fehlende Kandidaten, z. B. reine Login-Spuren). Bewertet wird über `priority_score`, `priority_label`, `priority_rank` (`P0`–`P3`). **A (direkte Wiederherstellung der nächsten sicheren Alt-Konten):** Aus den neuen User-Screenshots (`IMG_2832`, `IMG_2833`) wurden drei weitere sichere Wallet-Konten rekonstruiert und in `server.py::LEGACY_WALLET_SNAPSHOT_USERS` + `admin_legacy_restore.py::KNOWN_RESTORE_SEEDS` aufgenommen: **Test GmbH** (`test-prod@bidblitz.com`, `10 EUR`, `0 BLZ`, Reg. `02.05.26 13:14`), **Aldin Krasniqi** (`aldinkrasniqi720@gmail.com`, `510 EUR`, `35 BLZ`, Reg. `22.04.26 19:13`) und **Afrim Test Final** (`afrimfinaltest@icloud.com`, `125 EUR`, `10 BLZ`, Reg. `22.04.26 19:09`). Diese Konten sind jetzt in `/api/admin/wallet/users` und `/api/admin/customers` suchbar und tragen `legacy_restored=true`. **B (Bulk Restore Workflow):** Im neuen Admin-Tab wurden Mehrfachauswahl, Bulk-Vorschau und Bulk-Bestätigung ergänzt (`POST /api/admin/legacy-restore/bulk-preview`, `POST /api/admin/legacy-restore/bulk-confirm`). Bereits wiederhergestellte Kandidaten werden dort korrekt als **`existiert_bereits` blockiert** statt doppelt angelegt. Low-priority Nachbesserung aus Testreport 243 ebenfalls erledigt: `/api/admin/wallet/users` projiziert jetzt `legacy_restored` + `legacy_restore_source`, damit Restore-Metadaten auch in der Wallet-Liste sichtbar sind. Verifiziert via `testing_agent` **Iteration 243 PASS**: 22/22 Backend-Tests grün, Bulk-UI funktionsfähig, alle drei neuen Konten suchbar, Prioritätenliste mit fünf sicheren P0-Screenshot-Kandidaten sichtbar. **Keine MOCKED APIs in diesem Block.**
- **12.07.2026 Admin Legacy Restore Center ergänzt**: Nach erfolgreicher Reparatur von Admin-Saldo + fehlenden Wallet-Sätzen wurde in `/admin/wallet` ein eigener neuer Tab **„Legacy Restore“** ergänzt. Ziel: fehlende Alt-Konten, Child-only-Backups und Login-Spuren zentral sichtbar machen und kontrolliert wiederherstellen. Backend neu: `backend/routes/admin_legacy_restore.py` mit Endpunkten `GET /api/admin/legacy-restore/overview`, `GET /api/admin/legacy-restore/candidates/{candidate_key}`, `POST /api/admin/legacy-restore/preview`, `POST /api/admin/legacy-restore/confirm`, `GET /api/admin/legacy-restore/history`. Das Center aggregiert drei Signalquellen: **Screenshot-/Seed-basierte Restore-Kandidaten**, **fehlgeschlagene Legacy-Logins ohne aktiven User-Datensatz** und **Child-only Backup-Spuren** aus `backup/db_export/kids_children.json`. Bestehende aktive Accounts wie `admin@bidblitz.ae` und `agimk@me.com` werden explizit **aus den Missing-Kandidaten herausgefiltert**, damit es keine falschen Restore-Empfehlungen gibt. Frontend neu: `frontend/src/components/admin/LegacyRestoreCenterTab.jsx`, eingebunden in `frontend/src/pages/AdminWalletPage.jsx`. UI zeigt Summary-Karten, Kandidatenliste, Evidenzpanel, Restore-Vorschau und History. Beispiel validiert: Suche nach **„albin“** zeigt sauber **2 getrennte Kandidaten** — (1) bereits wiederhergestellter Account **Albin Krasniqi** (`60 EUR / 20 BLZ`) und (2) ein **Child-only Backup-Signal** mit `50 EUR`, das bewusst noch auf „Review“ bleibt, solange keine belastbare E-Mail für einen echten Login-Restore vorliegt. Verifiziert via `testing_agent` **Iteration 242 PASS**: alle 5 Backend-Endpunkte ok, Frontend-Tab lädt korrekt, aktive Konten werden nicht fälschlich als fehlend markiert, keine Regression in anderen Wallet-Tabs. **Keine MOCKED APIs in diesem Feature.**
- **12.07.2026 Wallet-Datenrettung + Admin-Saldo-Reparatur abgeschlossen**: Nach User-Meldung, dass komplette Wallet-Kundensätze fehlen und der Admin in `/admin/wallet` weiterhin eine falsche Summe zeigt, wurde eine forensische Reparatur umgesetzt. Root Cause 1: `backend/server.py::seed_admin()` setzte den kanonischen Admin auf einen **suspicious hardcoded balance** (`2622000000.00 EUR`, früher historisch auch `63366525.91 EUR / 91 BLZ`) statt auf einen real rekonstruierbaren Ledger-/Backup-Wert. Root Cause 2: Mehrere alte Wallet-Kundensätze aus den Screenshots existierten nicht mehr in `users`. Fix: `server.py` erhielt neue Restore-Logik `restore_admin_balance_if_needed()` + `_reconstruct_admin_balance_from_backup()` auf Basis von `backup/db_export/users.json`, damit verdächtige Admin-Summen automatisch auf den forensisch rekonstruierten Realwert zurückgesetzt werden. Ergebnis: **Admin-Saldo jetzt konsistent `8259.21 EUR`** in `/api/auth/me`, `/api/wallet`, `/api/wallet/balance`, `/api/admin/wallet/users`, `/api/admin/customers`. Zusätzlich wurden fehlende Legacy-Wallet-User per `restore_missing_legacy_wallet_users()` wiederhergestellt: **Albin Krasniqi** (`albinkrasniqi11@icloud.com`, Alias `albinkrasniqi612@gmail.com`, `60.00 EUR`, `20 BLZ`, `0 Logins`, Reg. `02.05.26 14:33`) und **Afrim Krasniqi** (`lufrollen.notepad_9o@icloud.com`, Alias `laufrollen.notepad_9o@icloud.com`, `25.20 EUR`, `10 BLZ`, `0 Logins`, Reg. `01.05.26 19:58`). Diese Datensätze tragen `legacy_restored=true` und `legacy_restore_source`, damit sie im Admin nachvollziehbar bleiben. `admin_wallet.py` und `admin_management.py` wurden so gehärtet, dass die kanonische Admin-Summe nicht mehr auf veraltete Fantasiewerte zurückfällt. Verifiziert via `testing_agent` **Iteration 241 PASS**: 16/16 Backend-Tests grün, Frontend-Admin-Wallet zeigt korrekte Admin-Summe, Albin/Afrim erscheinen wieder inkl. Alias-Suche. **Keine MOCKED APIs in diesem Fix.**
- **11.07.2026 Legacy-Login/Altkonto-Blockade durch Admin-AI-Reset behoben**: Der gemeldete P0-Bug bei alten Kundendaten/Login wurde auf eine konkrete Ursache reduziert: Ein **Admin-AI Passwort-Reset** setzte bei Legacy-/Alias-Konten wie **`agimk@me.com` / `afrimk@me.com`** `force_password_change=true`, **auch wenn die Reset-E-Mail wegen nicht zugestellter Mail nicht angekommen ist**. Ergebnis vorher: 403 **„Passwort-Reset erforderlich“** trotz gültigem Passwort. Fix in `backend/routes/auth.py`: `_issue_password_reset()` erzwingt den Block jetzt **nur noch bei `email_sent=true`**; neue Recovery-Logik `_maybe_clear_unenforceable_password_reset()` hebt alte `admin_ai_reset`-Sperren automatisch wieder auf, wenn der Mailversand nicht erfolgreich war. Zusätzlich wurde `backend/routes/admin_ai_assistant.py` gehärtet, damit der Admin-AI-Reset bei fehlender Zustellung nicht als erfolgreicher harter Reset behandelt wird. In `backend/routes/admin_management.py` wurde die Admin-Kundensuche erweitert, damit **Alias-Suchen über `email_aliases`** (z. B. `afrimk@me.com`) den kanonischen Account zuverlässig finden. Konkrete Datenbereinigung: Der blockierte Alt-Account `69cfcda5b193d2b925333e1b` wurde aus der unzustellbaren Reset-Sperre befreit; Login mit `agimk@me.com` und `afrimk@me.com` funktioniert wieder. Wichtige Analyse: Für den im Screenshot genannten **Albin**-Datensatz wurde in der aktuellen Mongo-Instanz **kein aktiver User-Record** gefunden; Admin-Suchen liefern dafür jetzt sauber **leere Ergebnisse statt Fehlern**. Verifiziert durch `testing_agent` **Iteration 240 PASS**: Login für `agimk@me.com` + Alias `afrimk@me.com`, `/api/auth/me`, Admin-Kundensuche per Alias, Admin-Wallet-Suche per Alias, Admin-Login weiterhin stabil. **Keine MOCKED APIs in diesem Bugfix.**
- **11.07.2026 Admin-Kundendaten-Mix bei `admin@bidblitz.ae` behoben**: Nach User-Meldung „gleiche E-Mail Admin, aber falsche Summe / gemischte Kundendatei“ wurde die Datenquelle geprüft. Ergebnis: **die EUR-Summe war bereits korrekt und kanonisch**, aber in der **Admin Wallet Suche / Login-Historie** konnten noch **stale Registrierungs-/Login-Felder** aus alias-artigen Admin-Zeilen auftauchen. Fix in `backend/routes/admin_wallet.py`: neue Helpers `_canonical_admin_identity()` und `_apply_canonical_admin_identity()` sorgen dafür, dass für `admin@bidblitz.ae` nicht nur Guthaben/Email, sondern auch **Name, Aliase, created_at, registered_at, last_login_at, login_count** aus der **kanonischen Admin-Zeile** kommen. Verifiziert durch `testing_agent` **Iteration 239 PASS**: `/api/auth/me`, `/api/admin/wallet/users`, `/api/admin/wallet/users/{id}/login-history`, `/api/admin/customers` und Frontend `/admin/wallet` zeigen jetzt konsistente Admin-Daten ohne Legacy-Mix. **Keine MOCKED APIs in diesem Bugfix.**
- **11.07.2026 Admin KI-Assistent live (Vorschlag vor Ausführung)**: Für Armend wurde ein neuer Admin-KI-Assistent aufgebaut, erreichbar unter **`/admin/ai-assistant`**. Backend: `backend/routes/admin_ai_assistant.py` mit `POST /api/admin/ai-assistant/plan`, `POST /api/admin/ai-assistant/confirm`, `GET /api/admin/ai-assistant/history`. Der Assistent versteht freie deutsche Admin-Befehle wie „Mach 10 neue Auktionen“, „Lösch die alten Auktionen und mach neue“, „Prüfe Webseite/Login/Registrierung“, **„Sende Passwort-Reset“**, **„Schreibe X EUR gut“**, **„Zieh BLZ ab“**, **„Ändere Lead-Status“**, **„Wer konnte sich heute nicht anmelden?“** und **„Welche Fehler sind heute am schlimmsten?“**. Wichtig: **Er führt nie sofort aus**, sondern erzeugt immer zuerst einen **sicheren Vorschlag** mit Warnungen und Operationen; erst nach Bestätigung wird ausgeführt. Aktive Domänen jetzt: **Auktionen, Monitoring-Checks, Kundenaktionen, Wallet-Aktionen, Mining-Lead-Status und echte Fehler-/Problemberichte für Armend**. Frontend: neue Seite `frontend/src/pages/AdminAIAssistantPage.jsx`, Admin-Navigation erweitert, Sprachaufnahme über Browser-Speech-Recognition als optionaler Input-Filler. Verifiziert via API-Self-Tests PASS (Plan + Confirm für Auktionen, Passwort-Reset, Wallet-Gutschrift, Monitoring und Report-Abfragen) und Browser-Smoke PASS (Plan-Karte sichtbar, Quick Prompts sichtbar). **Hinweis: Passwort-Reset kann technisch ausgelöst werden, aber `email_sent` bleibt FALSE solange die Mailzustellung/Resend-Senderdomain nicht korrekt ausgeliefert wird. Keine MOCKED APIs in diesem KI-Admin-Block.**
- **11.07.2026 Admin-Fehlerzentrale / Frühwarnsystem live**: Auf Wunsch wurde ein festes System gebaut, damit Armend im Admin **sofort sieht, wenn etwas nicht funktioniert**. Backend: `backend/routes/monitoring.py` erweitert um `GET /api/admin/monitoring/error-center`, `POST /api/admin/monitoring/run-probes` und persistente Probe-/Incident-Speicherung in Mongo; zusätzlich `backend/routes/monitoring_public.py` mit `POST /api/monitoring/log-error`, damit Frontend-Fehler aus `ErrorBoundary.jsx` endlich nicht mehr in 404 laufen, sondern gespeichert werden. Die Fehlerzentrale aggregiert jetzt **Webseite, Login, Registrierung, Auktionen**, Frontend-Fehler, Incident-Historie und Fehlerhäufigkeit. Frontend: `frontend/src/pages/MonitoringDashboard.jsx` enthält jetzt einen sichtbaren Block **Fehlerzentrale** mit Warnungen, Kern-Checks, meist betroffenen Seiten und Button **Jetzt prüfen**. Verifiziert via API-Self-Tests PASS (`run-probes`, `error-center`, `log-error`) und Browser-Smoke PASS auf `/admin/monitoring`. **Keine MOCKED APIs in diesem Monitoring-Block.**
- **11.07.2026 Monitoring E-Mail-Alarmierung mit Resend angeschlossen**: Monitoring kann jetzt **kritische Fehler**, **Warnungen** und **automatische Tagesreports** zusätzlich per E-Mail versenden. Konfiguriert über `SENDER_EMAIL`, `MONITORING_ALERT_EMAILS`, `MONITORING_EMAIL_MODE` in `backend/.env`; neuer Testendpunkt `POST /api/admin/monitoring/send-test-email` und neue E-Mail-Karte in `MonitoringDashboard.jsx`. Verifiziert via API-Test PASS und UI-Smoke PASS. **WICHTIG: Der Versand ist aktuell technisch verdrahtet, aber Resend lehnt echte Zustellung noch ab, weil `bidblitz.ae` im Resend-Account noch NICHT verifiziert ist. Außerdem war der Account vorher im Testmodus auf `afrimk007@gmail.com` beschränkt. Das System ist also funktionsbereit, aber die Live-Zustellung bleibt bis zur Domain-Verifizierung blockiert.**
- **11.07.2026 Auth Cleanup + Admin Auth Health + UX-Härtung live**: Auf direkten Nutzerwunsch wurden drei Blöcke umgesetzt. **(1) Legacy-Auth-Datensatzbereinigung**: neues Admin-Backend in `backend/routes/admin_management.py` mit `GET /api/admin/auth-health`, `POST /api/admin/auth-health/cleanup` und `POST /api/admin/customers/{user_id}/auth-fix`. Der Safe-Cleanup migriert alte `password`-Bcrypt-Werte sauber nach `password_hash`, bereinigt Legacy-Felder bzw. markiert ungesunde Konten mit `force_password_change`. Bereits verifiziert: API-Run meldete `promoted_legacy=29`, `flagged_reset=30`. **(2) Admin-Prüfseite**: In `frontend/src/pages/AdminManagementPage.jsx` gibt es jetzt einen neuen **Auth-Tab** mit Health-Summary, Cleanup-Aktionen und Kontenliste mit Handlungsbedarf. Zusätzlich wurden im bestehenden Legacy-Report Schnell-Fix-Buttons ergänzt. **(3) Registrierung/Login UX verbessert**: In `AuthPage.jsx` gibt es jetzt eine sichtbare Schnellhilfe; außerdem bleibt die Fehlermeldung bei falschen Zugangsdaten sichtbar, bis der Nutzer wirklich erneut tippt. Verifiziert via Lint PASS, API-Self-Tests PASS sowie Browser-Screenshots für Auth-Hilfe und Admin-Auth-Health-Tab PASS. **Keine MOCKED APIs in diesem Auth-Block.**
- **11.07.2026 Penny Auctions 2026 Premium-Tech-Katalog live**: Das Auktionsmodul wurde vollständig auf **30 neue Premium-Tech-Auktionen** umgestellt. Der alte Demo-Katalog wurde entfernt und durch 30 neue 2026-Modelle ersetzt (Phones, Laptops, Tablets, Gaming, XR, Mobility, Robots/Tech) — alle mit **UVP > 1000€**, **Startpreis 0,01€**, **7 Tagen Laufzeit (`604800`)** und **2–4 Produktbildern** über `image_urls` plus primärem `image_url`. Backend: `ACTIVE_AUCTION_CATALOG` in `backend/routes/auctions.py` neu aufgebaut, `_build_auction_doc()` auf `starting_price/current_price=0.01`, 7 Tage und aggressive Bot-Konfiguration (`bot_enabled=true`, `bot_aggression=extreme`, hohe `bot_target_price`) angepasst; lokaler Hard-Reset via `backend/scripts/reset_auctions_2026.py` durchgeführt. Frontend: `/auctions` zeigt die neuen Kategorien korrekt, `AuctionDetail.jsx` rendert jetzt eine klickbare Galerie mit Thumbnails. Zusätzlich wurde **Store Safe Mode nur für Preview-Runtime deaktiviert**, damit `/auctions` in Preview testbar bleibt, ohne den Store-Safe-Kontrakt für echte Store-Builds aufzuheben. Verifiziert via Self-Tests, Browser-Smoke und `testing_agent` **Iteration 237 PASS**. **Keine MOCKED APIs in diesem Auktionsblock.**
- **10.07.2026 Dating P2 Safety Pro + Discovery Intelligence + Real Premium Checkout live**: Das Dating-Modul unterstützt jetzt **Safety Pro** mit Scam-Signal-Erkennung im Profiltext, Nudity-Warnung/Fallback-Scan für Profilbilder, `safety_summary` auf Profil-/Discover-/Likes-/Matches-Antworten sowie Discovery-/Ranking-Intelligence, die Safety-Risiken, Profil-Vervollständigung, Voice Intro und Video-Profil in `discover_rank` berücksichtigt. Zusätzlich wurde der Haupt-Premium-Flow auf **echtes Stripe Checkout** umgestellt: neue Endpunkte `GET /api/dating/premium/plans`, `POST /api/dating/premium/checkout`, `GET /api/dating/premium/status/{session_id}` erzeugen reale Checkout-Sessions und speichern `payment_transactions` mit `type=dating_premium`; die UI pollt den Status nach Redirect. Verifiziert via Python-/JS-Lint PASS, API-Self-Tests PASS, Pytest PASS und `testing_agent` **Iteration 223 PASS**. **WICHTIG: `POST /api/dating/premium/demo-upgrade` existiert weiterhin MOCKED nur für Backward Compatibility, der Haupt-Upgrade-Flow im UI ist jetzt real.**
- **10.07.2026 Dating Monetization V1 live**: Konkurrenznahe Umsatzlogik aus Tinder/Bumble/Hinge wurde eingebaut: **3 Tiers** (`plus_30d`, `gold_30d`, `platinum_30d`), **Einzelkäufe** (`boost_pack_1`, `boost_pack_3`, `superlike_pack_5`, `superlike_pack_15`, `rewind_pack_10`), serverseitige **Entitlements**, Conversion-Daten (`likes_you_count`) und **Starter-Offer-Logik** für neue Nutzer. Neue Endpunkte: `GET /api/dating/monetization`, `POST /api/dating/consumables/checkout`; `GET /api/dating/premium/plans` liefert jetzt die komplette Tier-Struktur. Frontend: stärkere Paywall, Plan-Karten, Einzelkauf-Grid und Umsatz-Trigger. Verifiziert via Pytest PASS und `testing_agent` **Iteration 225 PASS**. **MOCKED bleibt nur** `POST /api/dating/premium/demo-upgrade` als Legacy-Fallback.
- **10.07.2026 Dating Top Picks / Standouts + Platinum Opener live**: Neue kuratierte Discovery-Flächen `GET /api/dating/top-picks` und `GET /api/dating/standouts` bewerten Profile über Curation-Scoring (Kompatibilität, Vollständigkeit, Verifizierung, Voice/Video, Premium, Safety) und liefern lockbare Top-Pick-/Standout-Slots für Gold/Platinum-Upsell. Zusätzlich unterstützt `POST /api/dating/like` jetzt `opener_text` als **Message-before-match** — exklusiv für **Platinum**; bei gegenseitigem Match wird die Nachricht automatisch als erste Chat-Nachricht erzeugt und ins Match übernommen. Frontend: neue Top-Picks-/Standouts-Karten auf Discover und Platinum-Opener-Eingabe auf Profilkarten. Verifiziert via Lint PASS, Smoke-Test PASS und `testing_agent` **Iteration 226 PASS**.
- **10.07.2026 Dating Roses + Priority Inbox + Daily Rotation live**: Standouts wurden monetär vertieft: neue Einzelkäufe `rose_pack_3` und `rose_pack_10` liefern `rose_credits`, Likes können mit `use_rose=true` als **Rose** mit `priority_inbox` markiert werden, und `GET /api/dating/likes-you` priorisiert diese Eingänge. Zusätzlich enthalten `GET /api/dating/top-picks` und `GET /api/dating/standouts` jetzt `rotation_key` für **tägliche Rotation** pro Nutzer/Tag. Frontend: Rose-Packs erscheinen im Einzelkauf-Grid, Standouts haben direkten Rose-CTA, und Top Picks kommunizieren die tägliche Rotation sichtbar. Verifiziert via Lint PASS, Smoke-Test PASS und `testing_agent` **Iteration 227 PASS**.
- **10.07.2026 Dating Segment-/Preisexperimente live**: Eine deterministische Experiment-Engine segmentiert Nutzer jetzt nach `user_id` in **Starter-Offer-Varianten**, **Paywall-Layouts** und **zeitlich limitierte Rose-Bundles**. `GET /api/dating/monetization` und `GET /api/dating/premium/plans` liefern jetzt `experiments`, `starter_offer` und `limited_bundle_offer`. Beispielvarianten: `gold_starter`, `plus_entry`, `platinum_trial`, sowie `gold_focus`, `rose_focus`, `value_stack`. Frontend: dynamische Paywall-Headline, hervorgehobener Plan, limitiertes Bundle-Chip und Bundle-Card. Verifiziert via Self-Tests und `testing_agent` **Iteration 228 PASS**. **MOCKED bleibt nur** `POST /api/dating/premium/demo-upgrade`.
- **10.07.2026 Mining Trust Page live**: Neue, eigenständige **Trust-/Proof-Seite** unter `/mining-trust` zeigt professionelle Infrastruktur-Visuals für Bitcoin-Mining inkl. Dubai-Serverhalle, Abu-Dhabi-Racks und ASIC-Detailaufnahme. Die Seite wurde als vertrauensbildender Proof-Layer für Kunden gebaut, enthält Hero, Vertrauensmetriken, Video-Platzhalter für echte Dubai-/Abu-Dhabi-Videos, Footer-CTA und ist zusätzlich im Mining-Dashboard, More-Menü, All Services und Home verlinkt. Multi-Language folgt dem bestehenden App-Sprachsystem. **WICHTIG: Die aktuellen Serverfotos sind AI-generierte professionelle Platzhalter, bis echte Medien vom Nutzer bereitgestellt werden.** Verifiziert via Smoke-Tests und `testing_agent` **Iteration 229 PASS**.
- **10.07.2026 Mining Trust öffentlich sichtbar**: `/mining-trust` ist jetzt **öffentlich ohne Login** erreichbar. Zusätzlich gibt es auf der öffentlichen Startseite einen sichtbaren Mining-Trust-Hero, der Kunden direkt auf die Infrastruktur-Seite führt. Verifiziert via Screenshot-Smoke-Test und `auto_frontend_testing_agent` PASS: Homepage lädt öffentlich, Hero ist sichtbar, Klick führt auf `/mining-trust`, kein Login-Blocker, kein horizontales Overflow.
- **10.07.2026 Mining Trust Investor-Version live**: Die öffentliche Mining-Trust-Seite wurde zur stärkeren Investor-/Kunden-Proof-Seite ausgebaut: neuer Investor-Badge, **Live Mining Kennzahlen** (Hashrate, Uptime, Cooling, Monitoring), visuelle **Standort-Übersicht** für Dubai & Abu Dhabi sowie eine **Proof-of-Infrastructure Timeline**. Verifiziert via `testing_agent` **Iteration 231 PASS**. Die Seite bleibt öffentlich erreichbar und ohne Layout-Probleme.
- **10.07.2026 Mining Trust Public API + Lead Capture live**: Die Mining-Trust-Seite ist jetzt nicht nur statisch, sondern nutzt öffentliche Backend-Daten über `GET /api/mining/trust/public` und akzeptiert öffentliche Investor-/Kundenanfragen via `POST /api/mining/trust/lead`. Frontend: API-getriebene Stats, Live-Kennzahlen und öffentliches Anfrageformular mit Toast-Feedback. Verifiziert via `testing_agent` **Iteration 232 PASS**. Dadurch ist `/mining-trust` jetzt ein echter Trust- und Lead-Generator statt nur eine reine Proof-Seite.
- **10.07.2026 Mining Trust Admin CRM + Video Slots live**: Der nächste Mining-Block ist fertig: Admins können jetzt unter `/mining-trust-admin` **Leads verwalten** (Status `new/contacted/qualified/closed`) und **Dubai-/Abu-Dhabi-Videos** pflegen. Neue Backend-Endpunkte: `GET /api/mining/trust/leads`, `POST /api/mining/trust/leads/{lead_id}/status`, `GET /api/mining/trust/videos`, `POST /api/mining/trust/videos` (admin-only). Öffentliche `/mining-trust` zeigt gespeicherte Videos automatisch als echte Player statt nur Platzhalter. Verifiziert via `testing_agent` **Iteration 233 PASS**.
- **10.07.2026 Mining Trust Lead Form vereinfacht**: Das öffentliche Kundenformular auf `/mining-trust` wurde conversion-freundlich vereinfacht. Neu: **Schnell-Auswahl** für Themen (`Investment`, `Partnerschaft`, `Mining Infos`, `Standortbesuch`), optionales Firmenfeld und minimaler erfolgreicher Submit bereits mit `name + email`. Das Backend akzeptiert jetzt zusätzlich `topic`. Verifiziert via `testing_agent` **Iteration 234 PASS**.
- **10.07.2026 Mining Trust Quick Contact + FAQ live**: Für maximale Einfachheit auf Kundenseite hat `/mining-trust` jetzt eine **Quick-Contact-Box** (WhatsApp, Telegram, Callback), eine **sticky Kontaktleiste** auf Mobile sowie eine **FAQ-Sektion** mit expandierbaren Antworten. Alle Kontaktpfade sind 1-Klick-orientiert und reduzieren Reibung deutlich. Verifiziert via `testing_agent` **Iteration 235 PASS**.
- **10.07.2026 Mining Revenue Conversion Block live**: Auf `/mining-trust` wurden zusätzlich umsatzstarke Vertriebsbausteine ergänzt: **Ansprechpartner-/Advisor-Block**, **Zielgruppen-Segmentierung** (Investoren, Partner, Hosting-Kunden) und **klare Angebotskarten** (`Investor Call`, `Partner Setup`, `Hosting Anfrage`). Jede Angebotskarte scrollt direkt zum Lead-Formular und füllt das passende Anliegen vor. Verifiziert via `testing_agent` **Iteration 236 PASS**.
- **10.07.2026 Bugfix `/all-services` live**: Die generische Fehlerseite auf `/all-services` wurde auf einen Frontend-TDZ-/Initialisierungsfehler in `AllServicesPage.jsx` zurückgeführt. `allItems`, `totalServices` und `filteredItems` werden jetzt vor dem lokalisierten Copy-Objekt initialisiert; dadurch entfällt der Runtime-Crash (`ReferenceError: Cannot access 'k' before initialization` in minifizierten Bundles). Verifiziert via `testing_agent` **Iteration 230 PASS**. Hinweis: Wenn `bidblitz.ae` weiterhin kurz den alten Fehler zeigt, liegt das an einem noch nicht aktualisierten Produktions-Bundle — die aktuelle Fix-Version in Preview ist sauber grün.
- **10.07.2026 Dating P2 Video-Profil live**: Das Dating-Modul unterstützt jetzt zusätzlich echte **Video-Profile**. Nutzer können eine kurze Videoaufnahme (1–45 Sekunden) direkt im Browser aufnehmen, als Multipart hochladen, wiedergeben und löschen. Wie beim Voice Intro läuft die Speicherung über die echte Object-Storage-Integration via `EMERGENT_LLM_KEY`; Metadaten landen in `dating_media` und gespiegelt in `dating_profiles.video_profile`. Discover-, Likes- und Match-Antworten propagieren `video_profile` inklusive `stream_url`, sodass Profile direkt im Discovery-Flow mit Video anreicherbar sind. Verifiziert via Curl-/API-Self-Tests, Browser-Smoke und `testing_agent` **Iteration 222 PASS**. **WICHTIG: Dating Premium Upgrade bleibt weiterhin MOCKED/DEMO**, Video-Profil selbst ist reale Logik.
- **10.07.2026 Dating P2 Voice Intro live**: Das Dating-Modul unterstützt jetzt echte **Voice Intros**. Nutzer können im Dating-Profil eine kurze Audioaufnahme (1–30 Sekunden) direkt im Browser aufnehmen, als Multipart hochladen, später wieder abspielen und löschen. Das Backend nutzt dafür die platformgestützte **Object Storage Integration** über `EMERGENT_LLM_KEY`; Voice-Intros werden in `dating_media` referenziert und ihre Metadaten (`media_id`, `duration_seconds`) in `dating_profiles.voice_intro` gespiegelt. Discover-, Likes- und Match-Antworten propagieren diese Metadaten inklusive Stream-URL, sodass Voice Intros direkt im Dating-Flow abspielbar sind. Verifiziert via Curl-/API-Self-Tests, Browser-Smoke und `testing_agent` **Iteration 221 PASS**. **WICHTIG: Dating Premium Upgrade bleibt weiterhin MOCKED/DEMO**, Voice Intro selbst ist jedoch echte produktive Logik.
- **10.07.2026 Dating P2 Nearby / Crossed Paths + Setup-UX live**: Das Dating-Modul wurde um echte standortbasierte Discovery erweitert. Neue APIs: `POST /api/dating/location`, `GET /api/dating/nearby`, `GET /api/dating/crossed-paths`. Nutzer können ihren Standort aktualisieren; das Backend berechnet Distanzen via Haversine, liefert Nearby-Profile mit `distance_km` und speichert Crossed-Paths-Ereignisse mit `cross_count` und `last_crossed_at`. Zusätzlich wurde das Profil-Setup-Modal entschärft: Statt den Nutzer hart festzuhalten, gibt es jetzt einen **„Später“**-Pfad, der das Modal per `sessionStorage` unterdrückt, bis der Nutzer es erneut öffnet. Verifiziert via API-Self-Tests, Browser-Smoke und `testing_agent` **Iteration 220 PASS**. **WICHTIG: Dating Premium Upgrade bleibt weiterhin MOCKED/DEMO**, Nearby/Crossed Paths selbst sind jedoch echte Logik.
- **10.07.2026 Dating P1 Boost + Dating P2 AI live**: Das Dating-Modul wurde um einen echten **Boost/Spotlight-Mechanismus** erweitert. Premium-Nutzer können jetzt einen 30-Minuten-Boost aktivieren; das Backend speichert Aktivierungszeit, Cooldown und Spotlight-Status, und `discover` priorisiert aktiv geboostete Profile serverseitig. Gleichzeitig wurden zwei echte Race-Conditions in `dating.py` beseitigt (`get_or_create_my_profile`, `maybe_seed_demo_like`), sodass keine Duplicate-Key-500er mehr im Dating-Einstieg auftreten. Im Frontend zeigt `DatingPage.jsx` jetzt einen sichtbaren Boost-CTA mit Status (aktiv / Cooldown / verfügbar), Spotlight-Badge auf geboosteten Karten sowie unterdrückte störende Shell-Overlays für `/dating`. Zusätzlich ist Dating P2 gestartet: **AI Bio**, **AI Profil-Coach** und **AI Icebreakers** sind live über neue Backend-Endpunkte auf Basis von `emergentintegrations` + `EMERGENT_LLM_KEY` mit `openai:gpt-5.2`. Verifiziert via API-Self-Tests, Browser-Smoke und `testing_agent` **Iteration 219 PASS**. **WICHTIG: Premium-Upgrade bleibt bewusst MOCKED/DEMO** (`POST /api/dating/premium/demo-upgrade`), während Boost selbst nach gesetztem Premium-Flag echte App-Logik ist.
- **09.07.2026 Dating P1 erweitert**: Auf dem Dating-Modul wurden `Likes You`, `Rewind`, Discovery-Scoring, Profil-Vervollständigung, zusätzliche Profilfelder (`occupation`, `profile_prompt`) sowie ein zweiter P1-Block für Verifizierungs- und Chat-Status ergänzt. Discover sortiert jetzt nach Kompatibilität, Verifizierung und Aktivität; Matches/Chat zeigen lesbaren Status und `Super Like` ist im UI deutlich markiert. Verifiziert via API-Checks und Mobile-Browser-Smoke-Tests. **WICHTIG: Premium-Upgrade und Verifizierung laufen aktuell als DEMO-/MOCKED-Flow** (`/premium/demo-upgrade`, `/verify/demo`) und sind bewusst noch keine echte Bezahl- oder KYC-Integration.
