...
## Current Focus — Pool Operator System Pricing & Access Control
- **16.07.2026 Schwimmbad-System auf Profi-Logik erweitert**: Das Pool-Modul wurde von einem MVP zu einer echten Betreiberlogik für **Tarife, Zeitfenster, Gästearten und Zutrittssteuerung** ausgebaut. Backend in `backend/routes/pool_management.py` unterstützt jetzt dynamische Preisberechnung über **`POST /api/pool/public/pricing/quote`** sowie Admin-Preisverwaltung über **`GET/POST /api/pool/admin/pricing/config`**. Abgedeckt sind **2 Stunden**, **Tagesticket**, **Abendticket**, **Erwachsene**, **Kinder**, **Familienkarte**, **Wochentag/Wochenende**, Extras mit **pro Buchung** oder **pro Gast**, sowie **Überziehungslogik** mit Grace-Period und Nachberechnung. Tickets speichern jetzt u. a. `duration_id`, `adult_count`, `child_count`, `valid_from`, `valid_until`, `access_zones`, `pricing_breakdown` und `overstay_fee`. Zusätzlich wurden echte Betreiber-Kommandos vorbereitet: **`POST /api/pool/admin/access/door-command`** für Tür-/Gate-Freigaben und **`POST /api/pool/admin/lockers/open`** für manuelle Spindöffnungen. Diese Kommandos landen sichtbar in einer neuen **Hardware Command Queue** (`pool_hardware_commands`) und werden im Hardware-Log gespiegelt. Frontend-seitig zeigt `/pool` jetzt die neue Preislogik mit Dauerwahl, Erwachsenen-/Kinderzählern, Besuchstag, Live-Preisaufschlüsselung und freigeschalteten Zutrittszonen. `/admin/pool` enthält neu einen **Preise-Tab** zum Anpassen von Wochentag-/Wochenendpreisen je Dauer und Extra-Preislogik sowie neue Einlass-/Spind-Steuerfelder für Tür öffnen und Spind sofort öffnen. **Wichtig:** Reale Hardware-Bridges für RFID, Turnstile und Locker bleiben weiterhin **MOCKED**, aber die Software-Verträge, Queue-Logik und Bedienoberflächen sind jetzt produktnah vorbereitet. Verifiziert durch Self-Tests und **Testing Agent Iteration 255 PASS**: Backend **24/24 Tests grün**, Frontend **100% PASS** für Pricing-Tab, Door Commands, Locker Open Commands und Hardware Command Queue.
