    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          ...
          <p className="text-[11px] leading-relaxed text-slate-700">4-stellige PIN für sichere Zahlungen und High-Risk-Freigaben.</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-300 bg-slate-100 p-3 sm:p-4">
        <p className="text-[10px] uppercase tracking-[0.1em] text-slate-600 mb-2">Schnell prüfen</p>
        <div className="flex flex-col sm:flex-row gap-2">
          <input value={verifyPin} onChange={(event) => setVerifyPin(event.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="PIN prüfen" inputMode="numeric" className="flex-1 min-h-[48px] rounded-2xl border border-slate-300 bg-white px-3 py-2 text-center text-base font-semibold tracking-[0.14em] sm:tracking-[0.28em] text-slate-950 placeholder:text-slate-500 focus:border-[#00D26A] focus:outline-none" style={{ WebkitTextFillColor: '#0f172a' }} data-testid="wallet-pin-verify-input" />
          <button type="button" onClick={verify} disabled={busy} className="min-h-[48px] rounded-2xl border border-[#00B85C] bg-[#00D26A] px-4 py-2 text-sm font-bold text-slate-950 disabled:opacity-50" data-testid="wallet-pin-verify-button">Prüfen</button>
        </div>
      </div>
    </motion.div>
  );
    ...
    <motion.div className="rounded-[26px] border p-4 sm:p-5 shadow-[0_18px_50px_rgba(15,23,42,0.08)]" style={{ background: "#ffffff", borderColor: "rgba(15,23,42,0.12)" }} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onViewportEnter={() => {
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          ...
          <p className="text-[11px] leading-relaxed text-slate-700">Nur verschlüsselte Template-Token. Keine biometrischen Bilder.</p>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-2 mb-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[120px,1fr] gap-2 mb-2">
        ...
        <input value={templateToken} onChange={(event) => setTemplateToken(event.target.value.trim())} placeholder="Template-Token registrieren" className="min-h-[48px] rounded-2xl border border-slate-300 bg-slate-100 px-3 py-2 text-sm text-slate-950 placeholder:text-slate-500 focus:border-[#7df4d2] focus:bg-white focus:outline-none" style={{ WebkitTextFillColor: '#0f172a' }} data-testid="wallet-biopay-enroll-token-input" />
      </div>
      <div className="flex gap-2 mb-3">
      ...
      <div className="rounded-2xl border border-slate-300 bg-slate-100 p-3 mb-3">
        <div className="flex items-center gap-2 mb-2"><Fingerprint size={14} className="text-[#0f9f74]" /><p className="text-[10px] uppercase tracking-[0.1em] text-slate-600">Schnell prüfen</p></div>
        <div className="flex flex-col sm:flex-row gap-2">
          <input value={verifyToken} onChange={(event) => setVerifyToken(event.target.value.trim())} placeholder="Template-Token prüfen" className="flex-1 min-h-[48px] rounded-2xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 placeholder:text-slate-500 focus:border-[#7df4d2] focus:outline-none" style={{ WebkitTextFillColor: '#0f172a' }} data-testid="wallet-biopay-verify-token-input" />
          <button type="button" onClick={verify} disabled={busy} className="min-h-[48px] rounded-2xl border border-[#39C89F] bg-[#52E0B9] px-4 py-2 text-sm font-bold text-slate-950 disabled:opacity-50" data-testid="wallet-biopay-verify-button">Prüfen</button>
        </div>
      </div>
