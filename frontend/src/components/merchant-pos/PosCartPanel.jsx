          ...
          <div key={item.product_id} className="rounded-[20px] border border-white/10 bg-[#071019] p-4" data-testid={`merchant-pos-cart-item-${index + 1}`}>
            ...
            <div className="mt-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <button onClick={() => onDecrease(item.product_id)} disabled={locked || item.quantity <= 1} className="inline-flex min-h-12 min-w-12 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 disabled:opacity-50" data-testid={`merchant-pos-cart-decrease-${index + 1}`} aria-label={`${item.name} reduzieren`}><Minus size={16} aria-hidden="true" /></button>
                ...
                <button onClick={() => onIncrease(item.product_id)} disabled={locked} className="inline-flex min-h-12 min-w-12 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 disabled:opacity-50" data-testid={`merchant-pos-cart-increase-${index + 1}`} aria-label={`${item.name} erhöhen`}><Plus size={16} aria-hidden="true" /></button>
              </div>
