...
export default function SponsoredAdSlot({ onNavigate }) {
  ...
  const clickAd = async () => {
    try { fetch(`${API}/api/pro/ads/click/${ad.ad_id}`, { method: "POST" }); } catch {}
    if (ad.link_route) onNavigate && onNavigate(ad.link_route);
  };

  return (
    <div className="mb-4" data-testid="sponsored-slot">
      <p className="text-[9px] text-white/30 uppercase tracking-wider mb-1 ml-1">Anzeige</p>
      <AnimatePresence mode="wait">
        <motion.button
          ...
          style={{
            background: `linear-gradient(135deg, ${ad.color || "#00C2FF"}25, ${ad.color || "#00C2FF"}08)`,
            border: `1px solid ${ad.color || "#00C2FF"}40`,
          }}
          data-testid={`ad-${ad.ad_id}`}
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: `${ad.color || "#00C2FF"}30` }}>
              <Megaphone size={18} style={{ color: ad.color || "#00C2FF" }}/>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-black text-white truncate">{ad.title}</p>
              {ad.description && (
                <p className="text-[11px] text-white/70 truncate mt-0.5">{ad.description}</p>
              )}
              {ad.merchant_name && (
                <p className="text-[9px] text-white/40 mt-0.5">von {ad.merchant_name}</p>
              )}
            </div>
            {ad.link_route && <ChevronRight size={15} className="text-white/40"/>}
          </div>
          {/* Dots */}
          {ads.length > 1 && (
