...
import { ArrowLeft, Check, FileText, Loader2, ShieldCheck, AlertCircle } from "lucide-react";
...
export default function InvoicePayPage({ scanCode, onNavigate }) {
  ...
  const payInvoice = async () => {
    ...
    setError("");
    try {
      const r = await fetch(`${API}/api/invoicing/public/${scanCode}/pay`, {
        method: "POST",
        credentials: "include",
      });
      const d = await r.json();
            ...
            <div className="px-6 py-5 space-y-3">
              ...
              {!user.isAuthenticated ? (
                <div className="space-y-3" data-testid="invoice-pay-login-box">
                  <p className="text-[11px] text-white/60 text-center mb-2">Melde dich an, um die Rechnung mit Wallet zu bezahlen</p>
                ...
                <div className="space-y-3" data-testid="invoice-pay-wallet-box">
                  ...
                  {error && <p className="text-[11px] text-red-400 text-center">{error}</p>}
                  <motion.button whileTap={{ scale: 0.97 }} onClick={payInvoice} disabled={paying || insufficient} className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#FF6B6B] to-[#FF8E53] text-[#020408] font-black text-sm flex items-center justify-center gap-2 disabled:opacity-40" data-testid="invoice-pay-confirm-btn">
                    {paying ? <Loader2 size={14} className="animate-spin" /> : `Jetzt ${total.toFixed(2)} EUR bezahlen`}
                  </motion.button>
