      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="text-center py-8">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#00C2FF]/20 to-[#A855F7]/20 flex items-center justify-center mx-auto mb-4">
              <Bot size={32} className="text-[#00C2FF]" />
            </div>
            <h2 className="text-lg font-bold text-white mb-1">Hallo! Ich bin BlitzBot</h2>
            <p className="text-xs text-gray-500 mb-6">Dein KI-Finanzassistent. Frag mich zu deinen Ausgaben, Spartipps oder Kontostand.</p>
            <div className="space-y-2">
              {quickQuestions.map((q, i) => (
                <motion.button key={i} whileTap={{ scale: 0.97 }}
                  onClick={() => { setInput(q); }}
                  className="block w-full text-left px-4 py-3 rounded-xl bg-white/[0.03] border border-white/5 text-sm text-white/70 hover:bg-white/[0.06] transition-colors"
