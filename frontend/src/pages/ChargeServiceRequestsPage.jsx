...
export default function ChargeServiceRequestsPage({ registrationId, onBack }) {
  ...
  const respondToProposal = useCallback(async (requestId, action) => {
    setBusy(`respond-${requestId}`);
    try {
      await api.respondChargeServiceRequest(requestId, { action });
      toast.success(action === "accept" ? "Servicetermin bestätigt" : "Neuer Terminvorschlag angefordert");
      await load();
    } catch (error) {
      toast.error(error.message || "Terminvorschlag konnte nicht beantwortet werden");
    } finally {
      setBusy("");
    }
  }, [load]);
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex shrink-0 flex-col gap-2">
                        {item.status === "reschedule_requested" ? (
                          <>
                            <button
                              onClick={() => respondToProposal(item.request_id, "accept")}
                              disabled={busy === `respond-${item.request_id}`}
                              className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-black text-emerald-700 disabled:opacity-50"
                              data-testid={`charge-service-accept-${index}`}
                            >
                              Termin annehmen
                            </button>
                            <button
                              onClick={() => respondToProposal(item.request_id, "decline")}
                              disabled={busy === `respond-${item.request_id}`}
                              className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-[11px] font-black text-amber-700 disabled:opacity-50"
                              data-testid={`charge-service-decline-${index}`}
                            >
                              Anderen Termin wünschen
                            </button>
                          </>
                        ) : null}
                        {cancellable ? (
                          <button
                            ...
                            className="rounded-full border border-red-200 bg-red-50 px-3 py-1 text-[11px] font-black text-red-700 disabled:opacity-50"
